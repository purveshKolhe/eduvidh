...
Source: https://modal.com/docs/examples/llm_inference
...
This has complicated their interface far beyond âtext-in, text-outâ.
...
For more on how to optimize performance, see [this guide](https://modal.com/docs/guide/high-performance-llm-inference) and check out our [LLM Engineerâs Almanac](https://modal.com/llm-almanac).
...
Weâll be running a pretrained foundation model â [Googleâs Gemma 4](https://blog.google/innovation-and-ai/technology/developers-tools/gemma-4/).
It can also take images, video, and audio as inputs,
though we wonât use that here.

Weâll use the 26BA4B variant, [`google/gemma-4-26B-A4B-it`](https://huggingface.co/google/gemma-4-26B-A4B-it).
...
Weâll use [Modal Volumes](https://modal.com/docs/guide/volumes) for our cache.
Modal Volumes are essentially a âshared diskâ that all Modal Functions can access like itâs a regular disk.
...
### Trading off fast boots and token generation performanceÂ
...
If youâre running an LLM service that frequently scales from 0 (frequent [âcold startsâ](https://modal.com/docs/guide/cold-start))
you might want to set this to `True`, or consider [GPU memory snapshots](https://modal.com/docs/guide/memory-snapshots).
Itâs also useful to set this when youâre iterating on the server configuration.

If youâre running an LLM service that usually has multiple replicas running, then set this to `False` for improved performance.

See the code below for details on the parameters that `FAST_BOOT` controls.

### Model-specific configurationÂ
...
and we activate the [built-in multi-token prediction (MTP)](https://blog.google/innovation-and-ai/technology/developers-tools/multi-token-prediction-gemma-4/) speculative decoding for improved throughput at lower concurrencies.
...
To make it easier to test the server setup, we also include a `local_entrypoint` that does a healthcheck and then hits the server. As opposed to Modal Functions, however
...
[Run OpenAI-compatible LLM inference with Gemma and vLLM](#run-openai-compatible-llm-inference-with-gemma-and-vllm)[Set up the container image](#set-up-the-container-image)[Download the model weights](#download-the-model-weights)[Configuring vLLM](#configuring-vllm)[Trading off fast boots and token generation performance](#trading-off-fast-boots-and-token-generation-performance)[Model-specific configuration](#model-specific-configuration)[Build a vLLM engine and serve it](#build-a-vllm-engine-and-serve-it)[Deploy the server](#deploy-the-server)[Interact with the server](#interact-with-the-server)[Testing the server](#testing-the-server)
