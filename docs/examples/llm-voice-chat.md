...
Source: https://modal.com/docs/examples/llm-voice-chat
...
[QuiLLMan](https://github.com/modal-labs/quillman)Â is a complete voice chat application built on Modal: you speak and the chatbot speaks back!

At the core is Kyutai Labâs [Moshi](https://github.com/kyutai-labs/moshi) model, a speech-to-speech language model that will continuously listen, plan, and respond to the user.
...
This page provides a high-level walkthrough of theÂ [GitHub repo](https://github.com/modal-labs/quillman).
...
On Modal, a function or class method can be exposed to web traffic by decorating it with [`@app.asgi_app()`](https://modal.com/docs/sdk/py/latest/modal.asgi_app#modalasgi_app) and returning a FastAPI app. Youâre then free to configure the FastAPI server however you like, including adding middleware, serving static files, and running websockets.

### Moshi Websocket ServerÂ
...
Kyutai Labâs [Moshi](https://github.com/kyutai-labs/moshi) bundles all modalities into one model, which decreases latency and makes for a much simpler app.
...
We use theÂ [Web Audio API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API)Â to record audio from the userâs microphone and playback audio responses from the model.
...
When youâre ready to go live, use the `deploy` command to deploy the App to Modal.
...
The code for this entire example isÂ [available on GitHub](https://github.com/modal-labs/quillman), so feel free to fork it and make it your own!
