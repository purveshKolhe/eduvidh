...
Source: https://modal.com/docs/examples/basic_web
...
This tutorial shows the path with the shortest [âtime to 200â](https://shkspr.mobi/blog/2021/05/whats-your-apis-time-to-200/): [`modal.fastapi_endpoint`](https://modal.com/docs/reference/modal.fastapi_endpoint).
...
## Turn a Modal Function into an API endpoint with a single decoratorÂ

Modal Functions are already accessible remotely â when you add the `@app.function` decorator to a Python function
...
Thatâs great, but itâs not much help if you want to share what youâve written with someone running code in a different language â
...
When youâre ready to deploy the Web Function persistently, run `modal deploy basic_web.py`.
Now, your Function will be available even when youâve closed your terminal or turned off your computer.

## Send data to a Web FunctionÂ
...
### Sending data in query parametersÂ

By default, your functionâs arguments are treated as query parameters:
...
### Sending data in the request bodyÂ
...
## Handle expensive startup with `modal.Cls`Â
...
itâd be a shame to have to do it every time a request comes in!
...
which allows you to manage the containerâs lifecycle independently from processing individual requests.
...
## Protect Web Functions with proxy authenticationÂ

Sharing your Python functions on the web is great, but itâs not always a good idea
...
To protect your Modal Web Functions so that they canât be triggered except
...
along with a âðâ emoji indicating that it is secured with proxy authentication.
...
so you can see that no containers were spun up to handle the request â this authorization is handled entirely inside Modalâs infrastructure.

You can trigger the Web Function by [creating a Proxy Auth Token](https://modal.com/settings/proxy-auth-tokens) and then including the token ID and secret in the `Modal-Key` and `Modal-Secret` headers.
...
Modalâs `fastapi_endpoint` decorator is opinionated and designed for relatively simple web applications â
...
[Hello world wide web!](#hello-world-wide-web)[Turn a Modal Function into an API endpoint with a single decorator](#turn-a-modal-function-into-an-api-endpoint-with-a-single-decorator)[Send data to a Web Function](#send-data-to-a-web-function)[Sending data in query parameters](#sending-data-in-query-parameters)[Sending data in the request body](#sending-data-in-the-request-body)[Handle expensive startup with modal.Cls](#handle-expensive-startup-with-modalcls)[Protect Web Functions with proxy authentication](#protect-web-functions-with-proxy-authentication)[What next?](#what-next)
