...
Source: https://modal.com/docs/examples/text_to_image
...
âA princess riding on a ponyâ.
...
the application. Letâs give it a friendly name.
...
## Configuring dependenciesÂ

The model runs remotely inside a [container](https://modal.com/docs/guide/custom-container).
That means we need to install the necessary dependencies in that containerâs image.

Below, we start from a lightweight base Linux image
and then install our Python dependencies, like Hugging Faceâs `diffusers` library and `torch`.
...
## Implementing SD3.5 Large Turbo inference on ModalÂ

We wrap inference in a Modal [Cls](https://modal.com/docs/guide/lifecycle-functions) that ensures models are loaded and then moved to the GPU once when a new container
...
## Generating Stable Diffusion images from the command lineÂ

This is the command weâll use to generate images. It takes a text `prompt`,
...
## Generating Stable Diffusion images via an APIÂ
...
## Generating Stable Diffusion images in a web UIÂ
