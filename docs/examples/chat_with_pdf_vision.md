...
Source: https://modal.com/docs/examples/chat_with_pdf_vision
...
âChat with PDFâ retrieval-augmented generation (RAG) app.
...
## Specifying the ColQwen2 modelÂ
...
## Managing state with Modal Volumes and DictsÂ
...
Thatâs what enables Modal Functions to automatically scale up and down [based on the number of incoming requests](https://modal.com/docs/guide/cold-start).

### Managing chat sessions with Modal DictsÂ
...
### Storing PDFs on a Modal VolumeÂ
...
## Defining a Chat with PDF serviceÂ

To deploy an autoscaling âChat with PDFâ vision-language model service on Modal,
...
to organize the âlifecycleâ of the app:
...
## Chatting with a PDF from the terminalÂ
...
[Chat with PDF: RAG with ColQwen2](#chat-with-pdf-rag-with-colqwen2)[Setup](#setup)[Setting up dependencies](#setting-up-dependencies)[Specifying the ColQwen2 model](#specifying-the-colqwen2-model)[Managing state with Modal Volumes and Dicts](#managing-state-with-modal-volumes-and-dicts)[Managing chat sessions with Modal Dicts](#managing-chat-sessions-with-modal-dicts)[Storing PDFs on a Modal Volume](#storing-pdfs-on-a-modal-volume)[Caching the model weights](#caching-the-model-weights)[Defining a Chat with PDF service](#defining-a-chat-with-pdf-service)[Loading PDFs as images](#loading-pdfs-as-images)[Chatting with a PDF from the terminal](#chatting-with-a-pdf-from-the-terminal)[A hosted Gradio interface](#a-hosted-gradio-interface)[Addenda](#addenda)
