...
Source: https://modal.com/docs/examples/hp_sweep_gpt
...
* [**Prompt Engineering**](https://en.wikipedia.org/wiki/Prompt_engineering):
  ...
  carefully design a natural language âpromptâ to elicit the desired behavior.
...
In fact, weâll train 8 SLMs in parallel with different hyperparameters
and then select the best one for additional training.

Weâll monitor this training live and serve our training and trained models
as Web Functions and simple browser UIs.

Along the way weâll use many features of the Modal platform: [distributed Volumes](https://modal.com/docs/guide/volumes),
...
Weâll use A10G GPUs for training, which are able to train the model to recognizably improved performance
...
### Create a Volume to store data, weights, and logsÂ

Since weâll be coordinating training across multiple machines weâll use a
distributed [Volume](https://modal.com/docs/guide/volumes) to store the data, checkpointed models, and TensorBoard logs.
...
### Define dependencies in container imagesÂ

The container image for training is based on Modalâs default slim Debian Linux image with `torch` for defining and running our neural network and `tensorboard` for monitoring training.
...
We also have some local dependencies that weâll need to import into the remote environment.
...
Weâll serve a simple Web Function:

```
web_image = base_image.uv_pip_install("fastapi[standard]==0.115.4", "starlette==0.41.2")
```

And weâll deploy a web UI for interacting with our trained models using Gradio.
...
We can also âpre-importâ libraries that will be used by the functions we run on Modal in a given image
...
## Running SLM training on ModalÂ
...
which `volume` to mount where, the `gpu` weâre using, and so on.
...
## Launch a hyperparameter sweep from a `local_entrypoint`Â

The main entry point coordinates the hyperparameter optimization.
First we specify the default hyperparameters for the model, taken from [Andrej Karpathyâs walkthrough](https://www.youtube.com/watch?v=kCc8FmEb1nY&t=5976s).
...
with different hyperparameters, varying the number of heads (`n_heads`), the `context_size` (called the âblock sizeâ by Karpathy), and the dropout rate (`dropout`). To run in
...
### Monitor experiments with TensorBoardÂ

To monitor our training we will create a TensorBoard WSGI web app, which will
display the progress of our training across all 8 models. Weâll use the latest
...
To ensure a unique color per experiment you can click the palette (ð¨) icon
...
You can also find some sample text generated by the model in the âTextâ tab.
...
## Serving SLMs on Modal during and after trainingÂ

Because our weights are stored in a distributed Volume,
we can deploy an inference Function based off of them without any extra work â
and we can even check in on models while weâre still training them! # For more on storing model weights on Modal, see [this guide](https://modal.com/docs/guide/model-weights).

### Remote inference with Modal `Cls`esÂ
...
### Adding a simple Web FunctionÂ

The `ModelInference` class above is available for use
from any other Python environment with the right Modal credentials
and the `modal` package installed â just use [`lookup`](https://modal.com/docs/reference/modal.Cls#lookup).
...
Itâs not exactly Shakespeare, but at least it shows our model learned something!

You can choose which model to use by specifying the `experiment_name` in the query parameters of the request URL.

### Serving a Gradio UI with `asgi_app`Â
...
even when weâre still training the model.
...
Thereâs quite a lot of code for just the training loop! If youâd rather not write this stuff yourself,
