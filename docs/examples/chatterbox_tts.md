...
Source: https://modal.com/docs/examples/chatterbox_tts
...
Check out [Resemble AIâs website](https://www.resemble.ai/) or
...
We start with Modalâs baseline `debian_slim` image and install the required packages.

* `chatterbox-tts`: The TTS model library
* `fastapi`: Web framework for creating the API endpoint
* âpeftâ: Required for properly loading the model
...
Weâll also use Chatterboxâs provided set of voice prompts which you can download [here](https://modal-cdn.com/blog/audio/chatterbox-tts-voices.zip).
...
Import the required libraries within the image context to ensure theyâre available
...
## The TTS model classÂ

The TTS service is implemented using Modalâs class syntax with GPU acceleration.
...
Weâll also need to provide a Hugging Face token using a `modal.Secret` to access the model weights,
...
Now deploy the Chatterbox API from this fileâs directory:
...
Youâll receive a WAV file named `/tmp/chatterbox-tts/output.wav` containing the generated audio.

[Create a Chatterbox TTS API on Modal](#create-a-chatterbox-tts-api-on-modal)[Setup](#setup)[Define a container image](#define-a-container-image)[The TTS model class](#the-tts-model-class)
