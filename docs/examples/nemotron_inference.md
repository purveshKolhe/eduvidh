...
Source: https://modal.com/docs/examples/nemotron_inference
...
In this example, we show how to serve Nvidiaâs [Nemotron](https://www.nvidia.com/en-us/ai-data-science/foundation-models/nemotron/) models
...
powerful capabilities in a model thatâs efficient to run.
...
which uses a [low-latency routing service on Modal](https://modal.com/blog/serverless-servers) designed for latency-sensitive inference workloads.
...
Weâll serve [NVIDIAâs Nemotron 3 Nano](https://arxiv.org/abs/2512.20856).
This model has 30 billion parameters, 3 billion of which are active per token.
For lower latency (in both [memory-bound](https://modal.com/gpu-glossary/perf/memory-bound) and [compute-bound](https://modal.com/gpu-glossary/perf/compute-bound) settings),
we choose the version quantized to [4 bit precision floating point](https://modal.com/llm-almanac/quant-formats).
...
Loading fewer bytes of model weights also speeds up [cold starts](https://modal.com/docs/guide/cold-start) of our inference server.
...
You can create a a Modal Secret with your Hugging Face token [here](https://modal.com/secrets). Make sure to name if `huggingface-secret`!
...
and supports both 8 bit and 4 bit [quantized floating point](https://modal.com/llm-almanac/quant-formats) operations.
...
You can find [interactive Swagger UI docs](https://swagger.io/tools/swagger-ui/) at the `/docs` route of that URL, i.e. `https://your-workspace-name--example-nemotron-inference-server.us-east.modal.direct/docs`.
...
[Low latency Nvidia Nemotron 3 with SGLang and Modal](#low-latency-nvidia-nemotron-3-with-sglang-and-modal)[Set up the container image](#set-up-the-container-image)[Loading and cacheing the model weights](#loading-and-cacheing-the-model-weights)[Define the inference server and infrastructure](#define-the-inference-server-and-infrastructure)[Selecting infrastructure to minimize latency](#selecting-infrastructure-to-minimize-latency)[Controlling container lifecycles with modal.Server](#controlling-container-lifecycles-with-modalserver)[Extra configuration](#extra-configuration)[Deploy the server](#deploy-the-server)[Interact with the server](#interact-with-the-server)[Test the server](#test-the-server)
