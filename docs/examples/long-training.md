...
Source: https://modal.com/docs/examples/long-training
...
In this example, weâll walk through how to implement this pattern in [PyTorch Lightning](https://lightning.ai/docs/pytorch/2.4.0/).
...
## Resuming from checkpoints in a training loopÂ
...
def train(experiment):
    ...
    if last_checkpoint.exists():
        print(f"â¡ï¸ resuming training from the latest checkpoint: {last_checkpoint}")
        train_model(
            DATA_PATH,
            experiment_dir,
            resume_from_checkpoint=last_checkpoint,
        )
        print("â¡ï¸ training finished successfully")
    else:
        print("â¡ï¸ starting training from scratch")
        train_model(DATA_PATH, experiment_dir)
```

This implementation works fine in a local environment.
Running it serverlessly and durably on Modal â with access to auto-scaling cloud GPU infrastructure
â does not require any adjustments to the code.
We just need to ensure that data and checkpoints are saved in Modal *Volumes*.

## Modal Volumes are distributed file systemsÂ

Modal [Volumes](https://modal.com/docs/guide/volumes) are distributed file systems â
...
## Porting training to ModalÂ

To attach a Modal Volume to our training function, we need to port it over to run on Modal.

That means we need to define our training functionâs dependencies
...
When running training jobs that last up to week, weâd set that timeout to 24 hours,
which would give our training job a maximum of 10 days to complete before weâd need to manually restart.

For this example, weâll set it to 30 seconds. When running the example, you should observe a few interruptions.
...
## Kicking off interruptible trainingÂ

We define a [`local_entrypoint`](https://modal.com/docs/guide/apps#entrypoints-for-ephemeral-apps) to kick off the training job from the local Python environment.
...
def main(experiment: Optional[str] = None):
    ...
    print(f"â¡ï¸ starting interruptible training experiment {experiment}")
    train_interruptible.spawn(experiment).get()
```

Itâs important to use `.spawn(...).get()` because `.remote` created Function Calls
...
## Details of PyTorch Lightning implementationÂ

This basic pattern works for any training framework or for custom training jobs â
