...
Source: https://modal.com/docs/examples/cloud_bucket_mount_loras
...
which produces the following image when prompted to generate âIKEA instructions for building a GPU rig for deep learningâ:
...
By the end of this example, weâve deployed a âplaygroundâ app where anyone with a browser can try
out these custom models. Thatâs the power of Modal: custom, autoscaling AI applications, deployed in seconds.
...
you now need to create a [Modal Secret](https://modal.com/docs/guide/secrets). Navigate to the âSecretsâ tab and
...
with the dependencies we need â no need to install them locally.
...
For the base model, weâll use a modal.Volume to store the Hugging Face cache.
...
where theyâll be accessible, at short latency and high throughput, for our Modal functions.
Downloading files in this mount will automatically upload files to S3.
To speed things up, we will run this function in parallel using Modalâs [`map`](https://modal.com/docs/reference/modal.Function#map).
...
## LoRA Exploradora: A hosted Gradio interfaceÂ
...
If youâre playing with the code, use `modal serve` instead to see changes live.
...
def ui():
    ...
    iface = gr.Interface(
        ...
        inputs=[  # the inputs to go/our inference function
            gr.Dropdown(choices=lora_ids, value=default_lora_id, label="ð LoRA ID"),
            gr.Textbox(default_prompt, label="ð¨ Prompt"),
            gr.Number(value=8888, label="ð² Random Seed"),
        ],
        ...
        "\n\nPowered by [Modal](https://modal.com) ð",
        theme="soft",
