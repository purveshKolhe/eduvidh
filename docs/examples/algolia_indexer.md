...
Source: https://modal.com/docs/examples/algolia_indexer
...
Letâs get the imports out of the way.
...
as long as they have `python` and `pip` available. Weâll use the official crawler image built by Algolia, with a small
...
Now, letâs configure the crawler with the website we want to index, and which
...
If you donât already have one, sign up for an account on [Algolia](https://www.algolia.com/). Set up
...
but we named it `algolia-secret` and so thatâs what the code below expects.

## The actual functionÂ

We want to trigger our crawler from our CI/CD pipeline, so weâre serving it as a [Web Function](https://modal.com/docs/guide/webhooks) that can be triggered by a `GET` request during deploy.
...
so weâre invoking it using a subprocess.
...
Thatâs all the code we need! To deploy your application, run
...
APP\_ID. Once youâre happy with the results, you can [set up the `docsearch` package with your
website](https://docsearch.algolia.com/docs/docsearch-v3/), and create a search component that uses this index.

## Entrypoint for developmentÂ
