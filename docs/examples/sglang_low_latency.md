...
Source: https://modal.com/docs/examples/sglang_low_latency
...
While weâre at it, we import the dependencies weâll need both remotely and locally (for deployment).
...
lowest precision well-supported in the relevant [GPU kernels](https://modal.com/gpu-glossary/device-software/kernel) across a variety of model architectures.
...
### Loading and cacheing the model weightsÂ

Weâll serve [Alibabaâs Qwen 3.6 LLM](https://qwen.ai/blog?id=qwen3.6).
...
Expert sparsity and lower precision reduce the amount of data that needs to be loaded [from GPU RAM into SM SRAM](https://modal.com/gpu-glossary/perf/memory-bandwidth) in each forward pass.
...
But we donât want to load the model from the Hub every time we start the server.
...
where we mount the Volume, we also [turn on âhigh performanceâ downloads](https://huggingface.co/docs/hub/en/models-downloading#faster-downloads),
...
### Cacheing compilation artifactsÂ

Model weights arenât the only thing we want to cache.

As a rule, LLM inference servers like SGLang donât directly provide their own kernels.
They draw high-performance kernels from a variety of sources.

As of version `0.5.12`, SGLangâs default kernel backend
for FP8 matrix multiplications (`fp8-gemm-backend`)
on Hopper [SM architecture](https://modal.com/gpu-glossary/device-hardware/streaming-multiprocessor-architecture) GPUs like the H100 is [DeepGEMM](https://github.com/deepseek-ai/DeepGEMM) by DeepSeek.
...
## Configure SGLang for minimal latencyÂ

LLM inference engines like SGLang come with a wide variety of âknobsâ to tune performance.

To determine the appropriate configuration to hit latency and throughput service objectives,
we recommend [application-specific benchmarking](https://modal.com/llm-almanac/how-to-benchmark) guided by [published generic benchmarks](https://modal.com/llm-almanac/advisor).
...
The primary contributor to per-request latency is the time to move all of the modelâs weights (multiple gigabytes)
from [GPU RAM](https://modal.com/gpu-glossary/device-hardware/gpu-ram) into [SRAM in the Streaming Multiprocessors](https://modal.com/gpu-glossary/device-hardware/streaming-multiprocessor),
which must be done at least once in the course of processing a request â
naively, once per token per request.
The time taken is limited by the [memory bandwidth](https://modal.com/gpu-glossary/perf/memory-bandwidth) between those two stores, which is on the order of terabytes per second on modern data center GPUs.
With models at the scale of gigabytes, a token will take milliseconds to generate â
...
### Increasing effective memory bandwidth with tensor parallelismÂ

Running SGLang on two H100s will double our effective [memory bandwidth](https://modal.com/gpu-glossary/perf/memory-bandwidth) during large matrix multiplications.
...
Actual speedups are generally less than what you get from ânapkin mathâ based on available bandwidths â
we observed a speedup of about 30% moving from one to two H100s when developing this example, rather than 100%.

### Parallelizing token generation with speculative decodingÂ

Transformer and recurrent language models generate text sequentially:
the modelâs output at step `i` is part of the input at step `i+1`.
Per Amdahlâs Law, that sequential work becomes the bottleneck
as other steps get faster from increased parallelism.

The solution is to generate more tokens on each step.
The primary technique to do so without changing model behavior is known as [*speculative decoding*](https://developer.nvidia.com/blog/an-introduction-to-speculative-decoding-for-reducing-latency-in-ai-inference/),
which âspeculatesâ a number of draft tokens and verifies them in parallel with the primary model.
...
Many models are released with built-in speculation based on [multi-token prediction](https://docs.vllm.ai/projects/ascend/en/main/user_guide/feature_guide/Multi_Token_Prediction.html),
also known in SGLang as [EAGLE](https://arxiv.org/abs/2401.15077).

But our favorite technique is [DFLASH](https://arxiv.org/abs/2602.06036) which runs draft token generation in parallel, increasing the draft modelâs
...
rather than `16`, the default. Weâre using the FP8 quantized model here
and the test prompts are creative writing tasks, so accept lengths
are generally below `8` and additional blocks donât have enough
...
speculative decoding is not good for [compute-bound](https://modal.com/gpu-glossary/perf/compute-bound) workloads, since it generally increases demand for [arithmetic bandwidth](https://modal.com/gpu-glossary/perf/arithmetic-bandwidth).
...
## Define the inference server and infrastructureÂ

 

### Selecting infrastructure to minimize latencyÂ

Minimizing latency requires geographic co-location of clients and servers.

So for low latency LLM inference services on Modal, you must select a [cloud region](https://modal.com/docs/guide/region-selection) for both the GPU-accelerated containers running inference
...
so they arenât shared across replicas.
...
Having a âwarmâ or âliveâ replica reduces latency by skipping slow initialization work
that occurs when new replica boots up (a [âcold startâ](https://modal.com/docs/guide/cold-start)).
...
However, since this is documentation code, weâll set it to `0` to avoid surprise bills during casual use.
...
in response to load. Without autoscaling, usersâ requests will queue
...
### Controlling container lifecycles with `modal.Server`Â
...
We use the [`requests` library](https://requests.readthedocs.io/en/latest/) to send ourselves these HTTP requests on [`localhost`/`127.0.0.1`](https://superuser.com/questions/31824/why-is-localhost-ip-127-0-0-1).
...
This will create a new App on Modal and build the container image for it if it hasnât been built yet.

## Interact with the serverÂ
...
You can find [interactive Swagger UI docs](https://swagger.io/tools/swagger-ui/) at the `/docs` route of that URL, i.e. `https://your-workspace-name--example-sglang-low-latency-sglang.us-west.modal.direct/docs`.
...
is starting up â timeouts on the client and 5XX responses from the server.
Modal returns the [503 Service Unavailable status](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/503) when a Modal Server has no live replicas.

We include a header with each request â `Modal-Session-ID`.
...
This ensures no container ends up as a âhot spotâ handling too many client requests.
...
[Low latency Qwen 3.6 with SGLang and Modal](#low-latency-qwen-36-with-sglang-and-modal)[Set up the container image](#set-up-the-container-image)[Loading and cacheing the model weights](#loading-and-cacheing-the-model-weights)[Cacheing compilation artifacts](#cacheing-compilation-artifacts)[Configure SGLang for minimal latency](#configure-sglang-for-minimal-latency)[Increasing effective memory bandwidth with tensor parallelism](#increasing-effective-memory-bandwidth-with-tensor-parallelism)[Parallelizing token generation with speculative decoding](#parallelizing-token-generation-with-speculative-decoding)[Define the inference server and infrastructure](#define-the-inference-server-and-infrastructure)[Selecting infrastructure to minimize latency](#selecting-infrastructure-to-minimize-latency)[Controlling container lifecycles with modal.Server](#controlling-container-lifecycles-with-modalserver)[Deploy the server](#deploy-the-server)[Interact with the server](#interact-with-the-server)[Test the server](#test-the-server)
