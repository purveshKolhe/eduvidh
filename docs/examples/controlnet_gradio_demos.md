...
Source: https://modal.com/docs/examples/controlnet_gradio_demos
...
ControlNet provides a minimal interface allowing users to use images to constrain StableDiffusionâs generation process.
...
We can only run one demo at a time, so this module avoids downloading the model and âdetectorâ dependencies for
all 10 demos and instead uses the demo configuration object to download only whatâs necessary for the chosen demo.

Even just limiting our dependencies setup to whatâs required for one demo, the resulting container image is *huge*.
...
## Setting up the dependenciesÂ

ControlNet requires *a lot* of dependencies which could be fiddly to setup manually, but Modalâs programmatic
...
3. `git`, so that we can download the ControlNet source code (thereâs no `controlnet` PyPi package)
4. some image process Linux system packages, including `ffmpeg`
5. and demo specific pre-trained model and detector `.pth` files

Thatâs a lot! Fortunately, the code below is already written for you that stitches together a working container image
ready to produce remarkable ControlNet images.

**Note:** a ControlNet model pipeline is [now available in Huggingfaceâs `diffusers` package](https://huggingface.co/blog/controlnet). But this does not contain the demo apps.
...
def download_demo_files() -> None:
    ...
    for url in demo.detector_files:
        ...
        print(f"download complete for {filepath}")
    print("ð finished baking demo file(s) into image.")
...
but the queueing and launched webserver arenât compatible with Modalâs serverless Web Function interface,
...
Serve your chosen demo app with `modal serve controlnet_gradio_demos.py`. If you donât have any images ready at hand,
try one thatâs in the `06_gpu_and_ml/controlnet/demo_images/` folder.

StableDiffusion was already impressive enough, but ControlNetâs ability to so accurately and intuitively constrain
