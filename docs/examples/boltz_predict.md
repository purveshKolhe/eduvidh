...
Source: https://modal.com/docs/examples/boltz_predict
...
## Fold a protein from the command lineÂ
...
This will set up the environment for running Boltz-2 inference in Modalâs cloud,
...
def main(
    ...
    print("ð§¬ loading model remotely")
    ...
    print(f"ð§¬ running boltz with input from {input_yaml_path}")
    output = boltz_inference.remote(input_yaml)

    output_path = Path("/tmp") / "boltz" / "boltz_result.tar.gz"
    output_path.parent.mkdir(exist_ok=True, parents=True)
    print(f"ð§¬ writing output to {output_path}")
    output_path.write_bytes(output)
```

 

## Installing Boltz-2 Python dependencies on ModalÂ

Code running on Modal runs inside containers built from [container images](https://modal.com/docs/guide/images) that include that codeâs dependencies.
...
## Storing Boltz-2 model weights on Modal with VolumesÂ

Not all âdependenciesâ belong in a container image. Boltz-2, for example, depends on
...
for any protein molecules. We pass the [âuse\_msa\_server](https://github.com/jwohlwend/boltz/blob/main/docs/prediction.md) flag to auto-generate the MSA using the mmseqs2 server.
...
def boltz_inference(boltz_input_yaml: str, args="") -> bytes:
    ...
    print(f"ð§¬ predicting structure using boltz model from {models_dir}")
    subprocess.run(
...
Hereâs the implementation of that function. For details, see our [guide to storing model weights on Modal](https://modal.com/docs/guide/model-weights).
...
def download_model(
    ...
    print(f"ð§¬ model downloaded to {models_dir}")
```
...
[Fold proteins with Boltz-2](#fold-proteins-with-boltz-2)[Setup](#setup)[Fold a protein from the command line](#fold-a-protein-from-the-command-line)[Installing Boltz-2 Python dependencies on Modal](#installing-boltz-2-python-dependencies-on-modal)[Storing Boltz-2 model weights on Modal with Volumes](#storing-boltz-2-model-weights-on-modal-with-volumes)[Running Boltz-2 on Modal](#running-boltz-2-on-modal)[Addenda](#addenda)
