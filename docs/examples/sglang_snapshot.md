...
Source: https://modal.com/docs/examples/sglang_snapshot
...
that have highly âburstyâ workloads, like document processing.
See [this guide](https://modal.com/docs/guide/high-performance-llm-inference) for a breakdown of different LLM inference workloads and how to optimize them.
...
Weâll serve [Alibabaâs Qwen 3 LLM](https://www.alibabacloud.com/blog/alibaba-introduces-qwen3-setting-new-benchmark-in-open-source-ai-with-hybrid-reasoning_602192).
For lower latency and faster cold starts, we pick a smaller model (8B params)
in a lower precision floating point format (FP8).
This reduces the amount of data that needs to be loaded [from GPU RAM into SM SRAM](https://modal.com/gpu-glossary/perf/memory-bandwidth) in each forward pass.
...
so weâll need their Python package.

```
sglang_image = sglang_image.uv_pip_install("huggingface-hub==0.36.0")
```

We donât want to load the model from the Hub every time we start the server.
...
As of version `0.5.6`, SGLangâs default kernel backend
...
the [Torch Inductor compiler](https://dev-discuss.pytorch.org/t/torchinductor-a-pytorch-native-compiler-with-define-by-run-ir-and-symbolic-shapes/747) with GPU snapshotting.
...
### Sleeping and waking an SGLang serverÂ

We prepare our SGLang inference server for snapshotting by first sending
a few requests to âwarm it upâ, ensuring that it is fully ready to process requests.
Then we âput it to sleepâ, moving non-essential data out of GPU memory,
with a request to `/release_memory_occupation`.
At this point, we can take a memory snapshot.
Upon snapshot restoration, we âwake upâ the server
...
* [`@modal.enter` and `@modal.exit`](https://modal.com/docs/guide/lifecycle-functions) to indicate
  which methods of the class should be run when starting the server and shutting it down. The `enter` methods also define what code is run before memory snapshot creation (`snap=True`) and after memory snapshot restoration (`snap=False`).

The `modal.concurrent` decorator and the lifecycle management are particular important
for bursty workloads and for snapshotting, respectively, so letâs discuss them in detail.

### Determining autoscaling policy with `@modal.concurrent`Â
...
We can set two values with the [`@modal.concurrent`](https://modal.com/docs/guide/concurrent-inputs) decorator. `max_inputs` should be set to the maximum number of inputs a replica can handle concurrently
without internal queueing â the `max-running-requests` in SGLang. `target_inputs` can be left unset or, if the per-request latency
...
Generally, this choice needs to be made as part of [LLM inference engine benchmarking](https://modal.com/llm-almanac/how-to-benchmark) in reference to a particular applicationâs latency and throughput targets.

### Controlling container lifecycles with `@modal.enter`Â
...
You can find [interactive Swagger UI docs](https://swagger.io/tools/swagger-ui/) at the `/docs` route of that URL, i.e. `https://your-workspace-name--example-sglang-snapshot-sglang.modal.direct/docs`.
...
[Serverless Qwen 3-8B with SGLang and Modal Snapshots](#serverless-qwen-3-8b-with-sglang-and-modal-snapshots)[Set up the container image](#set-up-the-container-image)[Loading and cacheing the model weights](#loading-and-cacheing-the-model-weights)[Cacheing compilation artifacts](#cacheing-compilation-artifacts)[Speed up cold starts with GPU snapshotting](#speed-up-cold-starts-with-gpu-snapshotting)[Sleeping and waking an SGLang server](#sleeping-and-waking-an-sglang-server)[Define the inference server and infrastructure](#define-the-inference-server-and-infrastructure)[Determining autoscaling policy with @modal.concurrent](#determining-autoscaling-policy-with-modalconcurrent)[Controlling container lifecycles with @modal.enter](#controlling-container-lifecycles-with-modalenter)[Deploy the server](#deploy-the-server)[Interact with the server](#interact-with-the-server)[Test the server](#test-the-server)[Test memory snapshotting](#test-memory-snapshotting)
