...
Source: https://modal.com/docs/examples/mochi
...
Hereâs one that we generated, inspired by our logo:
...
## Setting up the environment for MochiÂ
...
So weâll need to install `diffusers` and `transformers` from GitHub.
...
Weâll use this for saving our Mochi weights, as well as our video outputs.
...
## Setting up our Mochi classÂ

Weâll use the `@cls` decorator to define a [Modal Class](https://modal.com/docs/guide/lifecycle-functions) which we use to control the lifecycle of our cloud container.
