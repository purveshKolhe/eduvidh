...
Source: https://modal.com/docs/examples/blender_video
...
This example shows how you can render an animated 3D scene using [Blender](https://www.blender.org/)âs Python interface.
...
We need to define the environment each function runs in â its container image.
The block below defines a container image, starting from a basic Debian Linux image
adding Blenderâs system-level dependencies
and then installing the `bpy` package, which is Blenderâs Python API.
...
## Rendering a single frameÂ

We define a function that renders a single frame. Weâll scale this function out on Modal later.

Functions in Modal are defined along with their hardware and their dependencies.
This function can be run with GPU acceleration or without it, and weâll use a global flag in the code to switch between the two.
...
The details of the scene arenât too important for this example, but weâll load
...
## Combining frames into a videoÂ
...
## Rendering in parallel in the cloud from the comfort of the command lineÂ
...
We give the `local_entrypoint` two parameters to control the render â the number of frames to render and how many frames to skip.
