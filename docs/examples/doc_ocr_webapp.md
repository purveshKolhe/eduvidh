...
Source: https://modal.com/docs/examples/doc_ocr_webapp
...
Weâre going to build a simple âReceipt Parserâ web app that submits document parsing
...
Letâs get the imports out of the way and define an [`App`](https://modal.com/docs/reference/modal.App).
...
In `parse`, weâre going to submit tasks to the Function defined in the [Job
...
to get its result. If the call hasnât finished yet, we return a `202` status code, which indicates
...
Now that weâve defined our endpoints, weâre ready to host them on Modal.
First, we specify our dependencies â here, a basic Debian Linux
...
Then, we add the static files for our front-end. Weâve made [a simple React
