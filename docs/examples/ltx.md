...
Source: https://modal.com/docs/examples/ltx
...
## Storing data on Modal VolumesÂ
...
Weâll store the LTX-Video modelâs weights and the outputs we generate
on Modal Volumes.

We store the outputs on a Modal Volume so that clients
donât need to sit around waiting for the video to be generated.
...
We store the weights on a Modal Volume so that we donât
...
We donât have to change any of the Hugging Face code to do this â
we just set the location of Hugging Faceâs cache to be on a Volume
...
(from the Volume if itâs present or the Hub if itâs not)
...
## Generate videos from the command lineÂ
...
def main(
    ...
    def run():
        print(f"ð¥ Generating a video from the prompt '{prompt}'")
        start = time.time()
        ...
        print(f"ð¥ Client received video in {int(duration)}s")
        print(f"ð¥ LTX video saved to Modal Volume at {mp4_name}")

        local_dir = Path("/tmp/ltx")
        local_dir.mkdir(exist_ok=True, parents=True)
        local_path = local_dir / mp4_name
        local_path.write_bytes(b"".join(outputs.read_file(mp4_name)))
        print(f"ð¥ LTX video saved locally at {local_path}")

    run()

    if twice:
        print("ð¥ Generating a video from a warm container")
        run()
```
