...
Source: https://modal.com/docs/examples/db_to_sheet
...
In this tutorial, weâll show how to use Modal to schedule a daily report in a spreadsheet on Google Sheets
that combines data from a PostgreSQL database with data from an external API.

In particular, weâll extract the city of each user from the database, look up the current weather in that city,
and then build a count/histogram of how many users are experiencing each type of weather.

## Entering credentialsÂ

We begin by setting up some credentials that weâll need in order to access our database and output
...
Weâll now add another Secret for Google Sheets access through Google Cloud Platform. Click **New
secret** and select the Google Sheets preset.

In order to access the Google Sheets API, weâll need to create a *Service Account* in Google Cloud
Platform. You can skip this step if you already have a Service Account json file.

1. Sign up to Google Cloud Platform or log in if you havenât
...
5. Give the service account a suitable name, like âsheet-access-botâ. Click **Done**. You donât
...
Weâll name this other Secret `"gsheets-secret"`.
...
Weâll do that below by defining a variable to hold our Secret for accessing Postgres
...
In order to connect to the database, weâll use the `psycopg2` Python package. To make it available
to your Modal Function you need to supply it with an `image` argument that tells Modal how to
build the container image that contains that package. Weâll base it off of the `Image.debian_slim` base
image thatâs built into Modal, and make sure to install the required binary packages as well as
...
explicit credentials in your code. Weâll create a simple function that queries the city for each
...
## Applying Python logicÂ

For each row in our source data weâll run an online lookup of the current weather using the <http://openweathermap.org> API. To do this, weâll add the API key to
another Modal Secret. Weâll use a custom secret called âweather-secretâ with the key `OPENWEATHER_API_KEY` containing our API key for OpenWeatherMap.
...
Weâll make use of Modalâs built-in `function.map` method to create our report. `function.map` makes it really easy to parallelize work by executing a Function on every element in a sequence of
data. For this example weâll just do a simple count of rows per weather type â
answering the question âhow many of our users are experiencing each type of weather?â.
...
Letâs try to run this! To make it simple to trigger the function with some
predefined input data, we create a âlocal entrypointâ that can be
...
## Sending output to a Google SheetÂ

Weâll set up a new Google Sheet to send our report to. Using the âSharingâ dialog in Google
Sheets, share the document to the service accountâs email address (the value of the `client_email` field in the json file)
...
Lastly, we need to point our code to the correct Google Sheet. Weâll need the *key* of the document.
...
Weâll make use of the `pygsheets` python package to authenticate with
...
This entire app can now be deployed using `modal deploy db_to_sheet.py`. The [apps page](https://modal.com/apps) shows our cron jobâs execution history and lets you navigate to each invocationâs logs.
