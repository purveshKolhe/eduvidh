...
Weâll use `torch.compile` and NVIDIA H100 GPUs.

## Setting up the image and dependenciesÂ
...
Weâll make use of the full [CUDA toolkit](https://modal.com/docs/guide/cuda) in this example, so weâll build our container image off of the `nvidia/cuda` base.
...
For Hugging Faceâs [Diffusers](https://github.com/huggingface/diffusers) library
...
Later, weâll also use `torch.compile` to increase the speed further.
...
## Defining a parameterized `Model` inference classÂ
...
To generate an image we just need to call the `Model`âs `generate` method
...
def main(
    ...
    print(f"ð¨ first inference latency: {time.time() - t0:.2f} seconds")

    if twice:
        t0 = time.time()
        image_bytes = Model(compile=compile).inference.remote(prompt)
        print(f"ð¨ second inference latency: {time.time() - t0:.2f} seconds")

    output_path = Path("/tmp") / "flux" / "output.jpg"
    output_path.parent.mkdir(exist_ok=True, parents=True)
    print(f"ð¨ saving output to {output_path}")
    output_path.write_bytes(image_bytes)
```

 

## Speeding up Flux with `torch.compile`Â
...
Here, we follow the suggestions from Hugging Faceâs [guide to fast diffusion inference](https://huggingface.co/docs/diffusers/en/tutorials/fast_diffusion),
...
def optimize(pipe, compile=True):
    ...
    print("ð¦ running torch compilation (may take up to 20 minutes)...")
    ...
    print("ð¦ finished torch compilation")
