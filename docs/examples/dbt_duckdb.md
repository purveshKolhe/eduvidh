...
Source: https://modal.com/docs/examples/dbt_duckdb
...
Itâs comprised of the following:

* [DuckDB](https://duckdb.org) as the warehouseâs [OLAP](https://en.wikipedia.org/wiki/Online_analytical_processing) database engine
...
We also define the environment our application will run in â
...
Weâll also need to authenticate with AWS to store data in S3.
...
Create this Secret using the âAWSâ template from the [Secrets dashboard](https://modal.com/secrets).
...
Weâll walk through the rest of the steps below. See the `app.local_entrypoint` below for details.

Note that this is not the typical way that `seeds/` data is used, but itâs useful for this
...
## Run DBT on the cloud with ModalÂ

Modal makes it easy to run Python code in the cloud.
And DBT is a Python tool, so itâs easy to run DBT with Modal:
below, we import the `dbt` libraryâs `dbtRunner` to pass commands from our
Python code, running on Modal, the same way weâd pass commands on a command line.
...
After running the `run` command and seeing it succeed, check whatâs contained
under the bucketâs `out/` key prefix. Youâll see that DBT has run the transformations
defined in `sample_proj_duckdb_s3/models/` and produced output `.parquet` files.

## Serve fresh data documentation with FastAPI and ModalÂ
...
If you navigate to the output URL, you should see something like [![example dbt docs](/_app/immutable/assets/dbt_docs.BwfMuDI8.png)](https://modal-labs-examples--example-dbt-duckdb-serve-dbt-docs.modal.run)
...
The app will be served âserverlesslyâ â it will automatically scale up or down
during periods of increased or decreased usage, and you wonât be charged at all
when it has scaled to zero.

## Schedule daily updatesÂ

The following `daily_build` function [runs on a schedule](https://modal.com/docs/guide/cron) to keep the DuckDB data warehouse up-to-date. It is also deployed by the same `modal deploy` command for the docs app.

The source data for this warehouse is static,
so the daily executions donât really âupdateâ anything, just re-build. But this example could be extended
