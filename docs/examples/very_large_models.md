...
Source: https://modal.com/docs/examples/very_large_models
...
* use random âdummyâ weights when iteratively developing your server
...
See the [Modal Image guide](https://modal.com/docs/guide/images) for details.
...
After loading them we can cache these weights in a Modal [Volume](https://modal.com/docs/guide/volumes) so that they are loaded about 10x faster â
...
Both of these latencies kill productivity when youâre iterating
on aspects besides model behavior, like server configuration.

For this reason, we recommend skipping model loading while youâre developing
a server or configuration â even when benchmarking, if you can!
...
Weâll use Z.aiâs GLM 4.7 in eight bit [floating point quantization](https://modal.com/llm-almanac/quant-formats).
...
### Configure the inference engineÂ
...
Itâs designed to run GLM 4.7 FP8 at low to moderate concurrency.
In particular, it uses that modelâs built-in multi-token prediction speculative decoding to improve [time per output token](https://modal.com/llm-almanac/how-to-benchmark).
...
Youâll want to provide your own configuration file for other settings,
...
so much that downstream timeouts are triggered â
...
Here, weâll attach our Modal Image as the default for
...
Most importantly, we need to decide what hardware to run on. [H200 and B200 GPUs](https://modal.com/blog/introducing-b200-h200) have over 100 GB of [GPU RAM](https://modal.com/gpu-glossary/device-hardware/gpu-ram) â
141 GB and 180 GB, respectively.
The modelâs weights will be stored in this memory,
and they consume several hundred gigabytes of space,
so we will generally want several of these accelerators.
We also need space for the modelâs KV cache of activations
on input sequences.
...
Weâll use a Modal Server to serve our model.
...
Now weâre ready to put all of our infrastructure configuration
...
Thatâs defined below. It pings the `/health` endpoint until the server responds.
...
When youâre ready, you can create a persistent deployment with
...
[Serve very large language models (DeepSeek V3, Kimi-K2, GLM 4.7/5)](#serve-very-large-language-models-deepseek-v3-kimi-k2-glm-475)[Set up the container image](#set-up-the-container-image)[Load model weights](#load-model-weights)[Configure the inference engine](#configure-the-inference-engine)[Configure infrastructure](#configure-infrastructure)[Define the server](#define-the-server)[Test the server](#test-the-server)[Deploy the server](#deploy-the-server)[Addenda](#addenda)
