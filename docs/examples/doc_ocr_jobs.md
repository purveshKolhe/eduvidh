...
Source: https://modal.com/docs/examples/doc_ocr_jobs
...
Weâll use [Marker](https://github.com/datalab-to/marker) from [Datalab](https://www.datalab.to),
...
For the purpose of this tutorial, weâve also built a [React + FastAPI web app on Modal](https://modal.com/docs/examples/doc_ocr_webapp) that works together with it, but note that you donât need a web app running on Modal
...
Letâs first import `modal` and define an [`App`](https://modal.com/docs/reference/modal.App).
Later, weâll use the name provided for our job queue App to find it from our web app and submit tasks to it.
...
## Cache the pre-trained model on a Modal VolumeÂ
...
The `create_model_dict` function downloads model weights from Datalabâs
cloud storage (S3 bucket) if they arenât already present in the filesystem.
However, in Modalâs serverless environment, filesystems are ephemeral,
...
## Run Datalab Marker on ModalÂ

Now letâs set up the actual inference.
...
## Testing and debugging remote codeÂ
...
## Deploying the document conversion serviceÂ
...
Once itâs published, we can [look up](https://modal.com/docs/guide/trigger-deployed-functions) this Function
