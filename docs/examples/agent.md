...
Source: https://modal.com/docs/examples/agent
...
This example demonstrates how to build an LLM coding âagentâ that can generate and execute Python code, using
...
To create them, head to the [Secrets dashboard](https://modal.com/secrets), select âCreate new secretâ,
...
We execute the agentâs code in a Modal [Sandbox](https://modal.com/docs/guide/sandbox), which allows us to
run arbitrary code in a safe environment. In this example, we will use the [`transformers`](https://huggingface.co/docs/transformers/index) library to generate text with a pre-trained model. Letâs create a Sandbox with the necessary dependencies.
...
def run(code: str, sb: modal.Sandbox) -> tuple[str, str]:
    print(
        f"{COLOR['HEADER']}ð¦: Running in sandbox{COLOR['ENDC']}",
    ...
    if exc.returncode != 0:
        print(
            f"{COLOR['HEADER']}ð¦: Failed with exitcode {sb.returncode}{COLOR['ENDC']}"
        )
...
## Constructing the agentâs graphÂ
...
[Build a coding agent with Modal Sandboxes and LangGraph](#build-a-coding-agent-with-modal-sandboxes-and-langgraph)[Setup](#setup)[Creating a Sandbox](#creating-a-sandbox)[Constructing the agentâs graph](#constructing-the-agents-graph)[Running the Graph](#running-the-graph)
