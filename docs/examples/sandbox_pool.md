...
Source: https://modal.com/docs/examples/sandbox_pool
...
This example demonstrates how to build a pool of âwarmâ [Modal Sandboxes](https://modal.com/docs/guide/sandbox), and deploy a [Modal Web Function](https://modal.com/docs/guide/webhook-urls) that lets you claim
...
It uses a [Modal Queue](https://modal.com/docs/guide/dicts-and-queues#modal-queues) to store references to the warm Sandboxes, and functionality to maintain the pool
...
Each Sandbox is configured with a [readiness probe](https://modal.com/docs/guide/sandbox#readiness-probes) so we can
reliably wait for the server to be ready before adding it to the pool.

Itâs structured into two Apps:
...
The implementation borrows from [pawalt](https://github.com/pawalt)âs [Sandbox pool
example gist](https://gist.github.com/pawalt/7a505c38bba75cafae0780a5dd40e8b8). ð
...
2 minutes, meaning that if a Sandbox has less than 2 minutes left itâs considered
to be expiring too soon and will be terminated.

Youâll want to adjust these values depending on your use case.
...
### Adding a Sandbox to the poolÂ

This function creates and adds a new Sandbox to the pool. It waits for the
Sandboxâs readiness probe to pass before adding it, ensuring the server is
...
### Claiming a Sandbox from the poolÂ
...
It runs on a schedule to ensure the pool doesnât drift too far from the target size.
...
## Local commands for interacting with the poolÂ
...
### Check the current state of the poolÂ
...
### Claiming a Sandbox from the pool and print its URLÂ
...
### Run a demo of the Sandbox pool.Â
...
[Maintain a pool of warm Sandboxes that are healthy and ready to serve requests](#maintain-a-pool-of-warm-sandboxes-that-are-healthy-and-ready-to-serve-requests)[Main implementation](#main-implementation)[Health check](#health-check)[Adding a Sandbox to the pool](#adding-a-sandbox-to-the-pool)[Claiming a Sandbox from the pool](#claiming-a-sandbox-from-the-pool)[Maintaining the pool](#maintaining-the-pool)[Local commands for interacting with the pool](#local-commands-for-interacting-with-the-pool)[Deploy the app](#deploy-the-app)[Check the current state of the pool](#check-the-current-state-of-the-pool)[Claiming a Sandbox from the pool and print its URL](#claiming-a-sandbox-from-the-pool-and-print-its-url)[Run a demo of the Sandbox pool.](#run-a-demo-of-the-sandbox-pool)
