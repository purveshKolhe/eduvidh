...
Source: https://modal.com/docs/examples/vllm_throughput
...
the U.S. federal governmentâs Securities and Exchange Commission (SEC),
made available to the public for free in daily data dumps
via the SECâs Electronic Data Gathering, Analysis, and Retrieval System
...
further indicating we are close to the hardwareâs physical limits.
...
At Modalâs [current rates](https://modal.com/pricing) as of early 2026,
that comes out to roughly 4Â¢ per million tokens. [According to Artificial Analysis](https://artificialanalysis.ai/models/qwen3-8b-instruct),
API providers charge roughly five times as much for the same workload.

## Organizing a batch job on ModalÂ
...
While weâre at it, we import a bunch of the libraries we will need later.
...
Many batch jobs work nicely as scripts â code that is run
...
based on the `FunctionCall` ID â a simple string.
...
or put them in a Modal [Queue](https://modal.com/docs/guide/queues) or [Dict](https://modal.com/docs/guide/dicts) for later retrieval.
...
âRechunkingâ our data from a list of filings by day
...
Weâll use a lightweight Python `dataclass` to represent
each SEC `Filing`.

For our task, weâre going to take the `text` of a filing and produce
a `summary`. So the `text` is mandatory and the `summary` starts out empty (`None`),
to be filled in by the LLM.

Weâll also keep a bit of metadata that should be included.
But weâre not sure all of these fields will exist (API data is messy!),
...
letâs implement each component in turn.

## Serving tokens at maximum throughputÂ

First, the LLM service.

### Configuring vLLM for maximum throughputÂ

We choose the [vLLM](https://vllm.ai) inference engine. You might alternatively use [SGLang](https://docs.sglang.io).
...
so we attach a [Modal Volume](https://modal.com/docs/guide/volumes) to the locations where vLLM saves these files to ensure that they persist.
...
Weâll be running a smaller model in 8bit floating point format.
Hopper and later GPUs have native support for this format.
To maximize throughput, we want to ensure our inference is [compute-bound](https://modal.com/gpu-glossary/perf/compute-bound):
the bottleneck is not loading weights/KV cache from memory,
itâs performing computations on those values.
Roughly speaking, we want to be able to put together a batch
whose size is within an order of magnitude of the [ridge point arithmetic intensity](https://modal.com/gpu-glossary/perf/roofline-model) of the GPU for our floating point format, which is [~600 for an H100 SXM Tensor Code on FP8 data](https://modal.com/gpu-glossary/perf/arithmetic-intensity).

A single H100 GPU has enough [GPU RAM](https://modal.com/gpu-glossary/device-hardware/gpu-ram) for pretty large batches of this data for this model,
so we stick with one of those â and just one!
...
We select [FlashInfer](https://github.com/flashinfer-ai/flashinfer) as the provider of the attention kernels, which vLLM recommends
...
Critically, though, this means we donât get any results
until all of them are finished â a key engineering degree of freedom
for throughput-oriented offline/batch jobs!

We use a Modal [Cls](https://modal.com/docs/guide/lifecycle-functions) to control the spinup and shutdown logic for the `LLM` engine.
...
adding the modelâs output text to the `summary` field.
...
And thatâs it for the LLM portion of the pipeline!
...
## Transforming SEC filings for batch processingÂ

We can avoid having to deal directly with the low-level
details of the SECâs data format by using the [`edgartools` library](https://pypi.org/project/edgartools/).
...
Instead of hitting the SECâs EDGAR Feed API every time we want to run a job,
weâll cache the results for each day in a Modal Volume.
We use Modalâs [v2 Volumes](https://modal.com/docs/guide/volumes#volumes-v2-overview),
...
to a much lower value than the default of five minutes â here, five seconds.

## Loading filings from the SEC EDGAR FeedÂ
...
Weâll be downloading large (maybe megabytes to few gigabytes)
files with low concurrency, so thereâs little benefit to running an asynchronous web client.
...
We add [retries](https://modal.com/docs/guide/retries) via our Modal decorator as well, so that we can tolerate temporary outages or rate limits.
...
### Utilities for transforming SEC FilingsÂ
...
### Utilities for loading filings from the SEC EDGAR FeedÂ

The code in this section is used to load raw data from the Feed
section of SEC EDGAR.

Daily dumps are stored in [tar](https://www.math.utah.edu/docs/info/tar_4.html) archives, which the code below extracts.
...
[Run LLM inference at maximum throughput](#run-llm-inference-at-maximum-throughput)[Organizing a batch job on Modal](#organizing-a-batch-job-on-modal)[Serving tokens at maximum throughput](#serving-tokens-at-maximum-throughput)[Configuring vLLM for maximum throughput](#configuring-vllm-for-maximum-throughput)[Deploying vLLM on Modal](#deploying-vllm-on-modal)[Transforming SEC filings for batch processing](#transforming-sec-filings-for-batch-processing)[Loading filings from the SEC EDGAR Feed](#loading-filings-from-the-sec-edgar-feed)[Addenda](#addenda)[Utilities for transforming SEC Filings](#utilities-for-transforming-sec-filings)[Utilities for loading filings from the SEC EDGAR Feed](#utilities-for-loading-filings-from-the-sec-edgar-feed)
