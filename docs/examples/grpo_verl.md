...
* We then compute a reward for the modelâs solution using the reward function defined below.
* That reward value is then used to update the modelâs parameters according to the GRPO training algorithm.
...
## Defining the datasetÂ

In this example, weâll use reinforcement learning to train a model to solve math problems.
...
You will need to run this step if you donât already have data youâd like to use for this example.
...
We use a `default` reward function for GSM8K from the [verl repo](https://github.com/volcengine/verl/blob/v0.1/verl/utils/reward_score/gsm8k.py), modified to return 1.0 if itâs a correct answer and 0 otherwise.
...
If you wish to use Weights & Biases, as we do in this code, youâll need to create a Weights & Biases [Secret.](https://modal.com/docs/guide/secrets#secrets)
...
verl also keeps a separate control flow process thatâs independent of this, responsible for figuring out what step in the RL pipeline to execute.
...
## Performing inference on the trained modelÂ
...
[Train a model to solve math problems using GRPO and verl](#train-a-model-to-solve-math-problems-using-grpo-and-verl)[Setup](#setup)[Defining the image and app](#defining-the-image-and-app)[Defining the dataset](#defining-the-dataset)[Defining a reward function](#defining-a-reward-function)[Kicking off a training run](#kicking-off-a-training-run)[Performing inference on the trained model](#performing-inference-on-the-trained-model)
