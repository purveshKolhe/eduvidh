...
Source: https://modal.com/docs/examples/streaming_kyutai_stt
...
See the linked blog post for details on their âdelayed streamsâ architecture.
...
## Run Kyutai STT inference on ModalÂ

Now weâre ready to add the code that runs the speech-to-text model.
...
We also define multiple ways to access the underlying streaming STT service â
...
leads to a pretty big class! But thereâs not anything too complex here.
...
## Run a local Python client to test streaming STTÂ

We can test this code on the same production Modal infra
that weâll be deploying it on by writing a quick `local_entrypoint` for testing.
...
## Deploy a streaming STT service on the WebÂ

Weâve already written a Web backend for our streaming STT service â
thatâs the FastAPI API with the WebSocket in the Modal Cls above.

We can also deploy a Web frontend. To keep things almost entirely âpure Pythonâ,
...
[Stream transcriptions with Kyutai STT](#stream-transcriptions-with-kyutai-stt)[Setup](#setup)[Run Kyutai STT inference on Modal](#run-kyutai-stt-inference-on-modal)[Run a local Python client to test streaming STT](#run-a-local-python-client-to-test-streaming-stt)[Deploy a streaming STT service on the Web](#deploy-a-streaming-stt-service-on-the-web)
