...
Source: https://modal.com/docs/examples/fine_tune_asr
...
|  | Transcription |
| --- | --- |
| **Ground Truth** | âdeuterium you put into one element you make a new elementâ |
| **Prediction** | âthe theorem you put into one element you make a new elementâ |
...
| --- | --- |
| **Ground Truth** | âdeuterium you put into one element you make a new elementâ |
| **Prediction** | âdeuterium you put into one element you make a new elementâ |

Weâll use the âsmallâ subset of âScience and Technologyâ from the [GigaSpeech](https://huggingface.co/datasets/speechcolab/gigaspeech) dataset, which is enough data to see the model improve on scientific terms in just a
few epochs.

Note: GigaSpeech is a [gated model](https://huggingface.co/docs/hub/en/models-gated),
so youâll need to accept the terms on the [dataset card](https://huggingface.co/datasets/speechcolab/gigaspeech) and create a [Hugging Face Secret](https://modal.com/secrets/) to download it.

## SetupÂ

We start by importing our standard library dependencies, `fastapi`, and `modal`.

We also need an [`App`](https://modal.com/docs/guide/apps) object, which weâll use to
define how our training application will run on Modalâs cloud infrastructure.
...
Next weâll import the dependencies we need for the code that will run on Modal.
...
### Storing data on ModalÂ

We use [Modal Volumes](https://modal.com/docs/guide/volumes) for data we want to persist across function calls. In this case, weâll create a cache
...
saved, and where weâll load it from when deploying the model for inference.
...
### Defining our training FunctionÂ
...
models is âword error rateâ (WER):
...
## Calling a Modal function from the command lineÂ

The easiest way to invoke our training Function is by creating a `local_entrypoint` â
our `main` function that runs locally and provides a command-line interface to trigger
training on Modalâs cloud infrastructure.
...
## Deploying our fine-tuned model for inferenceÂ
...
We can use [modal.fastapi\_endpoint](https://modal.com/docs/reference/modal.fastapi_endpoint) to expose our inference function as a Web Function.
...
Hereâs an example of how to use this endpoint to transcribe an audio file:
...
[Fine-tune Whisper to Improve Transcription on Domain-Specific Vocab](#fine-tune-whisper-to-improve-transcription-on-domain-specific-vocab)[Setup](#setup)[Set up the container image](#set-up-the-container-image)[Storing data on Modal](#storing-data-on-modal)[Training](#training)[Defining our training Function](#defining-our-training-function)[Calling a Modal function from the command line](#calling-a-modal-function-from-the-command-line)[Deploying our fine-tuned model for inference](#deploying-our-fine-tuned-model-for-inference)[Support code](#support-code)
