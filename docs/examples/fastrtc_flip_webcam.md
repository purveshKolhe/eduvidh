...
Source: https://modal.com/docs/examples/fastrtc_flip_webcam
...
Itâs intended to help you get up and running with real-time streaming applications on Modal
as quickly as possible. If youâre interested in running a production-grade WebRTC app on Modal,
...
### Configure WebRTC streaming on ModalÂ
...
WebRTC provides low latency (âreal-timeâ) peer-to-peer communication
...
that information to the other peer trying to connect to it â in this case, a browser trying to share a webcam feed.
...
## Running a FastRTC app on ModalÂ
...
so we can host it on Modalâs cloud platform using the [`modal.asgi_app` decorator](https://modal.com/docs/guide/webhooks#serving-asgi-and-wsgi-apps) with [Modal Function](https://modal.com/docs/guide/apps).
...
Youâll want to tune these for your application!
...
You can also check on the applicationâs dashboard
via the `modal.com` URL thatappears below it.

The `modal serve` command produces a hot-reloading development server â
...
This FastRTC app is very much the âhello worldâ or âecho serverâ
of FastRTC: it just flips the incoming webcam stream and adds a âhelloâ message.
