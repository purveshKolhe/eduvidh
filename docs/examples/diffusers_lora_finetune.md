...
Source: https://modal.com/docs/examples/diffusers_lora_finetune
...
using a technique called textual inversion from [the âDreamboothâ paper](https://dreambooth.github.io/).
Effectively, it teaches a general image generation model a new âproper nounâ,
...
to building on large pretrained models using Modalâs building blocks, like [GPU-accelerated](https://modal.com/docs/guide/gpu) Modal Functions for compute-intensive work, [Volumes](https://modal.com/docs/guide/volumes) for storage,
...
### Downloading scripts and installing a git repo with `run_commands`Â

Weâll use an example script from the `diffusers` library to train the model.
...
### Configuration with `dataclass`esÂ
...
### Storing data created by our app with `modal.Volume`Â
...
## Low-Rank Adaptation (LoRA) fine-tuning for a text-to-image modelÂ

The base model we start from is trained to do a sort of âreverse [ekphrasis](https://en.wikipedia.org/wiki/Ekphrasis)â:
...
In this example, we âfinetuneâ Flux, making only small adjustments to the weights.
...
we change a much smaller matrix that works âalongsideâ the existing weights, nudging the model in the direction we want.
...
### Finetuning with Hugging Face ð§¨ Diffusers and AccelerateÂ

The model weights, training libraries, and training script are all provided by [ð¤ Hugging Face](https://huggingface.co).
...
which [despite having high GPU utilization](https://wandb.ai/cfrye59/dreambooth-lora-sd-xl/runs/ca3v1lsh/system) suffered from numerical instability during training and produced only black images â hard to debug without experiment management logs!
...
On the other hand, if the results donât look like your subject, you might need to increase `max_train_steps`.
...
we use Modalâs [container lifecycle](https://modal.com/docs/guide/lifecycle-functions) features, which require the function to be part
...
## Wrap the trained model in a Gradio web UIÂ

[Gradio](https://gradio.app) makes it super easy to expose a modelâs functionality
...
## Running your fine-tuned model from the command lineÂ

You can use the `modal` command-line interface to set up, customize, and deploy this app:

* `modal run diffusers_lora_finetune.py` will train the model. Change the `instance_example_urls_file` to point to your own petâs images.
...
def run(  # add more config params here to make training configurable
    ...
    print("ð¨ loading model")
    download_models.remote(SharedConfig())
    print("ð¨ setting up training")
