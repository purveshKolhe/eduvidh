...
Source: https://modal.com/docs/examples/vllm_snapshot
...
In this example, we show how to serve Mistralâs Ministral 3 vision-language models on Modal.
...
for long-running deployments by an order of magnitude using Modalâs [CPU + GPU memory snapshots](https://modal.com/docs/guide/memory-snapshot).
...
## Download the Ministral weightsÂ

We also need to download the model weights.
Weâll retrieve them from the Hugging Face Hub.

To speed up the model load, weâll toggle the `HIGH_PERFORMANCE` flag for Hugging Faceâs [Xet backend](https://huggingface.co/docs/hub/en/xet/index).
...
Weâll use the FP8 instruct variant of the 8B model:
...
like those of Modalâs [Hopper H100/H200 and Blackwell B200 GPUs](https://modal.com/blog/introducing-b200-h200).
...
### Cache with Modal VolumesÂ

Modal Functions are serverless: when they arenât being used,
...
We can preserve saved files by mounting a [Modal Volume](https://modal.com/docs/guide/volumes) â
a persistent, remote filesystem.

Weâll use two Volumes: one for weights from Hugging Face
...
### Improve cold start time with snapshotsÂ

Starting up a vLLM server can be slow â
...
using Modalâs [memory snapshots](https://modal.com/docs/guide/memory-snapshot),
which serialize the contents of CPU and GPU memory.

This adds quite some complexity to the code.
If youâre looking for a minimal example, see
our [`vllm_inference` example here](https://modal.com/docs/examples/vllm_inference).

Weâll need to set a few extra configuration values:
...
to toggle the server in and out of âsleep modeâ.
...
Weâll also need two helper functions.
...
It has `snap=False` so that it isnât included in the snapshot.
...
[Serverless Ministral 3 with vLLM and Modal](#serverless-ministral-3-with-vllm-and-modal)[Set up the container image](#set-up-the-container-image)[Download the Ministral weights](#download-the-ministral-weights)[Cache with Modal Volumes](#cache-with-modal-volumes)[Serve Ministral 3 with vLLM](#serve-ministral-3-with-vllm)[Improve cold start time with snapshots](#improve-cold-start-time-with-snapshots)[Define the server](#define-the-server)[Deploy the server](#deploy-the-server)[Interact with the server](#interact-with-the-server)[Test the server](#test-the-server)[Test memory snapshotting](#test-memory-snapshotting)
