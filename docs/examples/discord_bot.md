...
Source: https://modal.com/docs/examples/discord_bot
...
Here, we set up a simple [FastAPI app](https://fastapi.tiangolo.com/) to run that service and deploy it easily Modalâs [`@asgi_app`](https://modal.com/docs/guide/webhooks#serving-asgi-and-wsgi-apps) decorator.
...
The App is where weâll attach all the components of our bot.
...
an âAPI of APIsâ that returns information about free public APIs,
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(url) as response:
                ...
                message = (
                    f"# {data.get('emoji') or 'ð¤'} [{data['title']}]({data['source']})"
                )
...
and itâs nice to be able to interact with and test it in isolation.

For that, we add a `local_entrypoint` that calls the Modal Function.
Notice that we add `.remote` to the functionâs name.
...
## Integrate our Modal Function with Discord InteractionsÂ

Now we need to map this function onto Discordâs interface â
...
and a `token` that identifies the interaction (loosely, message) that weâre participating in.

So letâs write that out. This function doesnât need to live on Modal,
since itâs just encapsulating some logic â we donât want to turn it into a service or an API on its own.
That means we donât need any Modal decorators.
...
async def send_to_discord(payload: dict, app_id: str, interaction_token: str):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.patch(interaction_url, json=payload) as resp:
            print("ð¤ Discord response: " + await resp.text())
```
...
3. [Create a custom Modal Secret](https://modal.com/docs/guide/secrets) for your Discord bot.
   On Modalâs Secret creation page, select âDiscordâ. Copy your Discord applicationâs **Public Key** and **Application ID** (from the **General Information** tab in the Discord Developer Portal)
...
## Register a Slash CommandÂ

Next, weâre going to register a [Slash Command](https://discord.com/developers/docs/interactions/application-commands#slash-commands) for our Discord app. Slash Commands are triggered by users in servers typing `/` and the name of the command.
...
def create_slash_command(force: bool = False):
    ...
    if command_exists and not force:
        print(f"ð¤: command {command_description['name']} exists")
        return
    ...
    try:
        ...
        raise Exception("Failed to create slash command") from e
    print(f"ð¤: command {command_description['name']} created")
```

 

## Host a Discord Interactions endpoint on ModalÂ

If you look carefully at the definition of the Slash Command above,
youâll notice that it doesnât know anything about our bot besides an ID.
...
1. Weâll need to respond within five seconds or Discord will assume we are dead.
   Modalâs fast-booting serverless containers usually start faster than that,
   but itâs not guaranteed. So weâll add the `min_containers` parameter to our
   Function so that thereâs at least one live copy ready to respond quickly at any time.
   Modal charges a minimum of about 2Â¢ an hour for live containers (pricing details [here](https://modal.com/pricing)).
   Note that that still fits within Modalâs $30/month of credits on the free tier.
2. We have to respond to Discord that quickly, but we donât have to respond to the user that quickly.
   We instead send an acknowledgement so that they know weâre alive and they can close their connection to us.
   We also trigger our `reply` Modal Function, which will respond to the user via Discordâs Interactions API,
   but we donât wait for the result, we just `spawn` the call.
3. The protocol includes some authentication logic that is mandatory
   and checked by Discord. Weâll explain in more detail in the next section.
...
def web_app():
    ...
    async def get_api(request: Request):
        ...
        print(f"ð¤: unable to parse request with type {data.get('type')}")
        raise HTTPException(status_code=400, detail="Bad request")

    return web_app
```

The authentication for Discord is a bit involved and there arenât,
to our knowledge, any good Python libraries for it.

So we have to implement the protocol âby handâ.
...
The details arenât super important, but they appear in the `authenticate` function below
...
def authenticate(headers, body):
    ...
    print("ð¤: authenticating request")
...
Now that weâve walked through all of it,
weâre in a position to understand what those are
...
## Finish setting up Discord botÂ

To start using the Slash Command you just set up, you need to invite the bot to
a Discord server. To do so, go to your applicationâs **Installation** section on the [Discord Developer Portal](https://discord.com/developers/applications).
