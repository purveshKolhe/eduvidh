# Run Facebookâs Segment Anything Model 2 (SAM 2) on Modal | Modal Docs

Source: https://modal.com/docs/examples/segment_anything
...
# Run Facebookâs Segment Anything Model 2 (SAM 2) on Modal

This example demonstrates how to deploy Facebookâs [SAM 2](https://github.com/facebookresearch/sam2) on Modal. SAM2 is a powerful, flexible image and video segmentation model that can be used
...
## Set up dependencies for SAM 2Â
...
## Wrapping the SAM 2 model in a Modal classÂ
...
Weâll also use a modal Volume to cache the model weights so that they donât need to be downloaded
repeatedly when we start new containers. For more on storing model weights on Modal, see [this guide](https://modal.com/docs/guide/model-weights).
...
## Segmenting videos from the command lineÂ

Finally, we define a [`local_entrypoint`](https://modal.com/docs/guide/apps#entrypoints-for-ephemeral-apps) to run the segmentation from our local machineâs terminal.
...
## Helper functions for SAM 2 inferenceÂ
