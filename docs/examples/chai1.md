...
Source: https://modal.com/docs/examples/chai1
...
determining how the one-dimensional chain of amino acids encoded by DNA *folds* into a 3D object â
has emerged as a key application for machine learning and neural networks in biology.

In this example, we demonstrate how to run the open source [Chai-1](https://github.com/chaidiscovery/chai-lab/) protein structure prediction model on Modalâs flexible serverless infrastructure.
For details on how the Chai-1 model works and what it can be used for,
see the authorsâ [technical report on bioRxiv](https://www.biorxiv.org/content/10.1101/2024.10.10.615955).
...
def main(
    ...
    print(f"ð§¬ saving results to disk locally in {output_dir}")
...
## Installing Chai-1 Python dependencies on ModalÂ
...
## Storing Chai-1 model weights on Modal with VolumesÂ

Not all âdependenciesâ belong in a container image. Chai-1, for example, depends on
...
## Storing Chai-1 outputs on Modal VolumesÂ
...
def chai1_inference(
    ...
    print(
        f"ð§¬ done, results written to /{output_dir.relative_to('/preds')} on remote volume"
    )
...
* We use the `async` keyword here so that we can run the download for each model file
  as a separate task, concurrently. We donât need to worry about this use of `async` spreading to the rest of our code â Modal launches just this Function in an async runtime.
