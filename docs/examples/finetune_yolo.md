...
Source: https://modal.com/docs/examples/finetune_yolo
...
The popular âYou Only Look Onceâ (YOLO) model line provides high-quality object detection in an economical package.
...
of our code as part of the containerâs [image](https://modal.com/docs/guide/custom-container).
...
We also create a persistent [Volume](https://modal.com/docs/guide/volumes) for storing datasets, trained weights, and inference outputs. For more on storing model weights on Modal, see [this guide](https://modal.com/docs/guide/model-weights).
...
Weâll be downloading our data from the [Roboflow](https://roboflow.com/) computer vision platform, so to follow along youâll need to:
...
Youâre also free to bring your own dataset with a config in YOLOv10-compatible yaml format.

Weâll be training on the medium size model, but youâre free to experiment with [other model sizes](https://docs.ultralytics.com/models/yolov10/#model-variants).
...
## Run inference on single inputs and on streamsÂ

We demonstrate two different ways to run inference â on single images and on a stream of images.
...
To avoid it, we parallelize the disk reads across many workers using Modalâs [`.map`](https://modal.com/docs/guide/scale),
...
We use the `@enter` feature of [`modal.Cls`](https://modal.com/docs/guide/lifecycle-functions) to load the model only once on container start and reuse it for future inferences.
...
Weâll kick off our parallel training jobs and run inference from the command line.
