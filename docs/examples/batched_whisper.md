...
Source: https://modal.com/docs/examples/batched_whisper
...
In this example, we demonstrate how to run [dynamically batched inference](https://modal.com/docs/guide/dynamic-batching) for OpenAIâs speech recognition model, [Whisper](https://openai.com/index/whisper/), on Modal.
...
Letâs start by importing the Modal client and defining the model that we want to serve.
...
Weâll start with Modalâs baseline `debian_slim` image and install the relevant libraries.
...
Weâll define a function to download the model and cache it in a volume.
...
The inference function is best represented using Modalâs [class syntax](https://modal.com/docs/guide/lifecycle-functions).

We define a `@modal.enter` method to load the model when the container starts, before it picks up any inputs.
The weights will be loaded from the Hugging Face cache volume so that we donât need to download them when
...
## Transcribe a datasetÂ

In this example, we use the [librispeech\_asr\_dummy dataset](https://huggingface.co/datasets/hf-internal-testing/librispeech_asr_dummy) from Hugging Faceâs Datasets library to test the model.
...
async def transcribe_hf_dataset(dataset_name):
    ...
    print("ð Loading dataset", dataset_name)
    ds = load_dataset(dataset_name, "clean", split="validation")
    print("ð Dataset loaded")
    batched_whisper = Model()
    print("ð£ Sending data for transcription")
