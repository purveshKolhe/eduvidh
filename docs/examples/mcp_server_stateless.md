...
Source: https://modal.com/docs/examples/mcp_server_stateless
...
which is important for mapping onto Modalâs serverless Functions.
It uses the âstreamable HTTPâ transport type.

## Building the MCP serverÂ
...
This will be mounted by the FastAPI app, which we deploy as a [Modal Web Function](https://modal.com/docs/guide/webhooks) using [the `asgi_app` decorator](https://modal.com/docs/reference/modal.asgi_app):
...
make sure to select âStreamable HTTPâ as the âTransport Typeâ.

After connecting and clicking âList Toolsâ in the âToolsâ tab you should see your `current_date_and_time` tool listed, and if you âRun Toolâ it should give you the
...
[Deploy a remote, stateless MCP server on Modal with FastMCP](#deploy-a-remote-stateless-mcp-server-on-modal-with-fastmcp)[Building the MCP server](#building-the-mcp-server)[Testing the MCP server](#testing-the-mcp-server)[Deploying the MCP server](#deploying-the-mcp-server)
