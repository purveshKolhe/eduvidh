...
This guide covers resource configuration for both [Functions](/docs/guide/apps#apps-functions-and-entrypoints) and [Sandboxes](/docs/guide/sandboxes). For Sandbox-specific guidance on pricing and
...
behavior in linear algebra libraries (e.g., `OPENBLAS_NUM_THREADS`, `OMP_NUM_THREADS`, `MKL_NUM_THREADS`) based on your CPU request.
...
maximum will be rejected with an [`InvalidError`](/docs/sdk/py/latest/modal.exception#modalexceptioninvaliderror).

## BillingÂ

For CPU and memory, youâll be charged based on whichever is higher: your request or actual usage.
...
Modal containers can have a hard memory limit which will âOut of Memoryâ (OOM) kill
...
1. The size of the underlying workerâs SSD disk capacity
2. A per-container disk quota that defaults to 512 GiB.

Hitting either limit will cause the containerâs disk writes to be rejected, which
...
[Configuring CPU, memory, and disk](#configuring-cpu-memory-and-disk)[CPU cores](#cpu-cores)[Memory](#memory)[How much can I request?](#how-much-can-i-request)[Billing](#billing)[Resource limits](#resource-limits)[CPU limits](#cpu-limits)[Memory limits](#memory-limits)[Disk limits](#disk-limits)

See it in action

[Sandbox pricing and resources](/docs/guide/sandbox-resources)