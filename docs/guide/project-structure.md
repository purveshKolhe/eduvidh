...
Source: https://modal.com/docs/guide/project-structure
...
Say that you have a simple project thatâs distributed across three files:

```
src/
âââ app.py  # Defines the `modal.App` as a variable named `app`
âââ llm.py  # Imports `app` and decorates some functions
âââ web.py  # Imports `app` and decorates other functions
```

With this structure, if you deploy using `modal deploy src/app.py`, Modal wonât
...
### Defining your project as a Python packageÂ

A better approach would be to define your project as a Python *package* and to
use the Modal CLIâs âmodule modeâ invocation pattern.
...
Unfortunately, itâs not enough just to set this up and make your deploy command `modal deploy src/app.py`. Instead, you need to invoke Modal in *module mode*: `modal deploy -m src.app`. Note the use of the `-m` flag and the module path
...
Functions, itâs important to name your Modal Functions uniquely across the
project even when splitting it up across files: otherwise you risk some
Functions âshadowingâ others with the same name.

## Including local dependenciesÂ
...
should be all set. If youâre not using a package setup, or if the local
dependencies are external to your projectâs package, youâll need to explicitly
...
âautomountâ any local dependencies that were imported by your App source into a
