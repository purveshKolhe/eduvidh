...
Source: https://modal.com/docs/guide/sandbox
...
## What are Sandboxes and why should I use them?Â
...
Each individual job is called a **Sandbox** and can be created using the [`Sandbox.create`](/docs/sdk/py/latest/modal.Sandbox#create) constructor:
...
of a Modal container. You may pass in a regular `App` object or look one up by name with [`App.lookup`](/docs/sdk/py/latest/modal.App#lookup). The `create_if_missing` flag on `App.lookup` will create an `App` with the given name if it doesnât exist.
...
1. **Created** â The Sandbox has been requested and registered with Modal. At this
...
2. **Scheduled** â The Sandbox has been scheduled to a specific worker. The
...
3. **Started** â The Sandboxâs container has been launched on a worker and the
   ...
   mounts are active.
4. **Ready** â If [readiness probes](/docs/guide/sandboxes#readiness-probes) are
...
5. **Finished** â The Sandbox has stopped running. This can happen for several
   ...
   stopped running in the dashboard or by examining the exit code returned from `sandbox.poll()`.
...
If you need a Sandbox to run for more than 24 hours, we recommend using [Filesystem Snapshots](/docs/guide/sandbox-snapshots) to preserve its state,
...
After a Sandbox starts, you often need to run custom initialization logic before itâs
ready for use â pulling code with `git pull`, installing dependencies, starting a server,
writing config files, or other setup that isnât baked into the image. Readiness probes
give you a way to track when that initialization is complete, so you donât have to build
...
* **TCP probe** â Checks whether a TCP port inside the Sandbox is accepting connections.
  This is the most common choice when your startup logic includes launching a server.
* **Exec probe** â Runs an arbitrary command inside the Sandbox and succeeds when the
...
# The server is now ready â interact with it via tunnels, exec, etc.
sb.terminate()
sb.detach()
```
...
#### Exec readiness probeÂ

Use an exec probe when readiness depends on something other than a TCP port â for
...
timeout does **not** automatically terminate the Sandbox â you may want to catch the `TimeoutError` and explicitly terminate the Sandbox if readiness is never achieved:
...
#### Separating Image builds from Sandbox creationÂ

To avoid blocking creation of new Sandboxes on rebuilding an invalidated Image,
itâs recommended to use Modalâs named Images with sandboxes, rather than using
inline Image definitions.

Use [`Image.build`](/docs/sdk/py/latest/modal.Image#build) to trigger
Image builds as part of a deployment flow or at a regular interval (e.g., in a [scheduled job](/docs/guide/cron) or CI pipeline), then publish the result as a [named image](/docs/guide/named-images)

Sandboxes can then use [`Image.from_name`](/docs/sdk/py/latest/modal.Image#from_name) to reference the Image in a way thatâs guaranteed to not block on rebuilds.
...
## Running a Sandbox with an entrypointÂ
...
to keep running in the background. See our [Jupyter notebook example](/docs/examples/jupyter_sandbox) for a more concrete example of this.

## Referencing Sandboxes from other codeÂ
...
as they come in. You can keep a list of `object_id`s of Sandboxes that are âopenâ and
...
## Cleaning up Client-side ConnectionsÂ
...
[Sandboxes](#sandboxes)[What are Sandboxes and why should I use them?](#what-are-sandboxes-and-why-should-i-use-them)[Lifecycle](#lifecycle)[Events](#events)[Timeouts](#timeouts)[Idle Timeouts](#idle-timeouts)[Readiness Probes](#readiness-probes)[TCP readiness probe](#tcp-readiness-probe)[Exec readiness probe](#exec-readiness-probe)[Return Codes](#return-codes)[Configuration](#configuration)[Environments](#environments)[Environment variables](#environment-variables)[Custom Images](#custom-images)[Separating Image builds from Sandbox creation](#separating-image-builds-from-sandbox-creation)[Image build logs](#image-build-logs)[Running a Sandbox with an entrypoint](#running-a-sandbox-with-an-entrypoint)[Referencing Sandboxes from other code](#referencing-sandboxes-from-other-code)[Named Sandboxes](#named-sandboxes)[Tagging](#tagging)[Cleaning up Client-side Connections](#cleaning-up-client-side-connections)
...
[Serving OpenCode for remote agents](/docs/examples/opencode_server)