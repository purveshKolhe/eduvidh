...
Source: https://modal.com/docs/guide/notebooks-modal
...
Notebooks allow you to write and execute Python code in Modalâs cloud, within your browser. Itâs a hosted Jupyter notebook with:
...
Note that the CPU and memory requests configure the *minimum* amount of resources allocated, but you can usually burst above the request. For example, if youâve set the Notebook to have 0.5 CPU cores, youâll be billed for that continuously, but you can use up to any available cores on the machine (e.g., 32 CPUs) and will be billed for only the time you use them.

### Notebook pricingÂ

Modal Notebooks are priced simply, by compute usage while the kernel is running. See the [pricing page](https://modal.com/pricing) for rates. Currently the CPU and Memory costs are priced according to Sandboxes. They appear in your [usage dashboard](/settings/usage) under âNotebooksâ.

Inactive notebooks do not incur any cost. You are only billed for time the notebook is actively running.

## Custom images, volumes, secrets, and cloud storageÂ
...
### Creating a Custom ImageÂ

If you donât have a suitable deployed Modal App already, you can set up your environment to deploy custom images in under a minute using the Modal CLI. First, run `pip install modal`, and define your image in a file like:
...
(Advanced) Note that if you use the [`add_local_file()` or `add_local_dir()` functions](/docs/guide/images#add-local-files-with-add_local_dir-and-add_local_file), youâll need to pass `copy=True` for them to work in Modal Notebooks. This is because they skip creating a custom image and instead mount the files into the function at startup, which wonât work in notebooks.
...
2. In the notebook sidebarâs Files panel, use the Cloud Buckets section to attach your bucket
3. Specify:
   * The S3 bucket name
...
Notebooks are editable by you and teammates in your workspace. To make the notebook view-only to collaborators, the creator of the notebook can change access settings in the âShareâ menu. Workspace managers are also allowed to change this setting.

You can also turn on sharing by public, unlisted link. If you toggle this, it allows *anyone with the link* to open the notebook, even if they are not logged in. Pick **Can view** (default) or **Can view and run** based on your preference. Viewers donât need a Modal account, so this is perfect for collaborating with stakeholders outside your workspace.

No matter how the notebook is shared, anyone with access can fork and run their own version of it.

## Interactive file viewerÂ
...
| **Context-aware paths** | The viewer always reflects *exactly* what your code sees (e.g. `/root`, `/mnt/â¦`), so you can double-check that that file you just wrote really landed where you expected. |
...
The viewer itself is only active while the kernel is runningâif the notebook is stopped youâll see an âemptyâ state until you start it again.
...
We also implemented AI-powered code completion using Anthropicâs **Claude Sonnet 4.6** model. This keeps you in the flow for everything from small snippets to multi-line functions. Just press `Tab` to accept suggestions or `Esc` to dismiss them.
...
Finally, we have real-time collaborative editing, so you can work with your team in the same notebook. You can see other usersâ cursors and edits in real-time, and you can see when others are running cells with you. This makes it easy to pair program or review code together.
...
[Modal Notebooks](#modal-notebooks)[Getting started](#getting-started)[Kernel resources](#kernel-resources)[Notebook pricing](#notebook-pricing)[Custom images, volumes, secrets, and cloud storage](#custom-images-volumes-secrets-and-cloud-storage)[Creating a Custom Image](#creating-a-custom-image)[Creating a Secret](#creating-a-secret)[Creating a Volume](#creating-a-volume)[Mounting Cloud Buckets](#mounting-cloud-buckets)[Access and sharing](#access-and-sharing)[Interactive file viewer](#interactive-file-viewer)[Editor features](#editor-features)[Widgets](#widgets)[Cell magic](#cell-magic)[Roadmap](#roadmap)