...
Modalâs `@batched` feature allows you to accumulate requests
...
Modalâs dynamic batching waits for the lesser of a fixed time *or* a fixed number of requests before executing,
maximizing the throughput benefit of batching while minimizing the latency penalty.

## Enable dynamic batching with `@batched`Â

To enable dynamic batching, apply the [`@modal.batched` decorator](/docs/sdk/py/latest/modal.batched) to the target
...
## Use `@batched` with functions that take and return listsÂ
...
## Modal `Cls` methods are compatible with dynamic batchingÂ
...
## Configure the wait time and batch size of dynamic batchesÂ
...
### Selecting a batch configurationÂ
...
## Serve Web Functions with dynamic batchingÂ

Hereâs a simple example of serving a Function that batches requests dynamically
with a [`@modal.fastapi_endpoint`](/docs/guide/webhooks). Run [`modal serve`](/docs/cli/latest/serve), submit requests to the endpoint,
...
[Dynamic batching](#dynamic-batching)[Enable dynamic batching with @batched](#enable-dynamic-batching-with-batched)[Use @batched with functions that take and return lists](#use-batched-with-functions-that-take-and-return-lists)[Modal Cls methods are compatible with dynamic batching](#modal-cls-methods-are-compatible-with-dynamic-batching)[Configure the wait time and batch size of dynamic batches](#configure-the-wait-time-and-batch-size-of-dynamic-batches)[Selecting a batch configuration](#selecting-a-batch-configuration)[Serve Web Functions with dynamic batching](#serve-web-functions-with-dynamic-batching)