...
The document outlines Modalâs security and privacy commitments.

## Application security (AppSec)Â
...
* We use HTTPS for secure connections. Modal forces HTTPS for all services using
  ...
  connections. Modalâs [client library](https://pypi.org/project/modal) connects
  to Modalâs servers over TLS and verify TLS certificates on each connection.
...
## Network and infrastructure security (InfraSec)Â
...
## Vulnerability remediationÂ

Security vulnerabilities directly affecting Modalâs systems and services will be
...
credit card information for any of our customers. Stripe is certified as âPCI
Service Provider Level 1â, which is the highest level of certification in the
payments industry.
...
wish to participate, please send an email to [security@modal.com](mailto:security@modal.com) with your
...
| Request and response payloads | [Server](/docs/guide/servers) and [Auto Endpoints](/docs/guide/endpoints) | Not stored â proxied directly to your container |
...
#### Function inputs and outputsÂ

Function inputs and outputs are stored encrypted at rest. Small payloads
(â¤ 2 MiB) are stored inline in our metadata store; larger payloads are stored
in object storage. Both are deleted within a maximum TTL of 7 days.

#### Modal Inference endpointsÂ

Modal Inference endpoints are zero data retention: request and response payloads
are never written to disk and pass through Modalâs infrastructure only as
in-flight network traffic. Inference endpoints terminate TLS at Modalâs edge
