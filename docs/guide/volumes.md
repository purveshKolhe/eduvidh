...
* Volumes are distributed by default, so you can use them alongside Modalâs global compute pool without needing to manage replicas across regions.
...
For reference documentation on the `modal.Volume` object, see [this page](/docs/sdk/py/latest/modal.Volume).
For reference documentation on the `modal volume` CLI command, see [this page](/docs/cli/latest/volume).
...
> ð± To create a v2 Volume, pass `--version=2` in the command above.

## Using a Volume on ModalÂ
...
When attaching a Volume to a Function or Sandbox, you can configure mount options using [`Volume.with_mount_options`](/docs/sdk/py/latest/modal.Volume#with_mount_options).
These options are not stored on the Volume itself â they apply per container mount,
...
### Mounting a sub-pathÂ

You can mount a subdirectory of a Volume instead of the entire Volume using the `sub_path` mount option.
If the subdirectory doesnât exist yet, it will be created when the container starts.
...
multiple end user sessions, but donât want a session to access or even see
...
## Downloading a file from a VolumeÂ
...
### Creating Volumes lazily from codeÂ
...
> ð± To create a v2 Volume, pass `version=2` to the call to `from_name()` in the code above.

This will create the Volume if it doesnât exist.

## Using a Volume from outside of ModalÂ
...
### Using a Volume from local codeÂ
...
### Using a Volume via the command lineÂ
...
## Volume commits and reloadsÂ

Unlike a normal filesystem, you need to explicitly reload the Volume to see
changes made since it was first mounted. This reload is handled by invoking the [`.reload()`](/docs/sdk/py/latest/modal.Volume#reload) method on a Volume object.
...
the [`.commit()`](/docs/sdk/py/latest/modal.Volume#commit) method on a `modal.Volume` object.
...
running container, that Volume modification wonât become available until the
...
job from the last saved checkpoint. This is particularly helpful in managing [preemption](/docs/guide/preemption).

For more, see our [example code for long-running training](/docs/examples/long-training).

### Hugging Face `transformers`Â

To periodically checkpoint into a `modal.Volume`, just set the `Trainer`âs [`output_dir`](https://huggingface.co/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments.output_dir) to a directory in the Volume.
...
## Filesystem consistencyÂ

 

### Concurrent modificationÂ
...
concurrent modification of the same file â any data the last writer didnât have
...
open files on the Volume the [`.reload()`](/docs/sdk/py/latest/modal.Volume#reload) operation will fail with âvolume busyâ. The following is a simple example of how
a âvolume busyâ error can occur:
...
### Canât find file on Volume errorsÂ

When accessing files in your Volume, donât forget to pre-pend where your Volume
is mounted in the container.

In the example below, where the Volume has been mounted at `/data`, âhelloâ is
...
### In v2, you can store as many files as you wantÂ
...
### In v2, you can write concurrently from hundreds of containersÂ
...
### In v2, random accesses have improved performanceÂ
...
### In v2, you can commit using `sync`Â
...
This is useful when you donât have access to the Python SDKâs [`.commit()`](/docs/sdk/py/latest/modal.Volume#commit) method, such as when running
...
and then persist all data and metadata changes to the Volumeâs persistent
storage.
...
> â ï¸ This feature is only available for Volumes v2.

### Volumes v2 has a few limits in placeÂ
...
use a tool like `cp` if you intend to copy all the data in one go, or `rsync` if you want to incrementally copy the data across a longer time span:
...
[Volumes](#volumes)[Pricing](#pricing)[Volumes v2](#volumes-v2)[Creating a Volume](#creating-a-volume)[Using a Volume on Modal](#using-a-volume-on-modal)[Mount options](#mount-options)[Read-only mounts](#read-only-mounts)[Mounting a sub-path](#mounting-a-sub-path)[Downloading a file from a Volume](#downloading-a-file-from-a-volume)[Creating Volumes lazily from code](#creating-volumes-lazily-from-code)[Using a Volume from outside of Modal](#using-a-volume-from-outside-of-modal)[Using a Volume from local code](#using-a-volume-from-local-code)[Using a Volume via the command line](#using-a-volume-via-the-command-line)[Volume commits and reloads](#volume-commits-and-reloads)[Background commits](#background-commits)[Model serving](#model-serving)[Model checkpointing](#model-checkpointing)[Hugging Face transformers](#hugging-face-transformers)[Volume performance](#volume-performance)[Filesystem consistency](#filesystem-consistency)[Concurrent modification](#concurrent-modification)[Busy Volume errors](#busy-volume-errors)[Canât find file on Volume errors](#cant-find-file-on-volume-errors)[Disk usage reporting](#disk-usage-reporting)[Volumes v2 overview](#volumes-v2-overview)[Volumes v2 are still in Beta](#volumes-v2-are-still-in-beta)[Volumes v2 are HIPAA compliant](#volumes-v2-are-hipaa-compliant)[Volumes v2 is more scaleable](#volumes-v2-is-more-scaleable)[In v2, you can store as many files as you want](#in-v2-you-can-store-as-many-files-as-you-want)[In v2, you can write concurrently from hundreds of containers](#in-v2-you-can-write-concurrently-from-hundreds-of-containers)[In v2, random accesses have improved performance](#in-v2-random-accesses-have-improved-performance)[In v2, you can commit using sync](#in-v2-you-can-commit-using-sync)[Volumes v2 has a few limits in place](#volumes-v2-has-a-few-limits-in-place)[Upgrading v1 Volumes](#upgrading-v1-volumes)[Further examples](#further-examples)
