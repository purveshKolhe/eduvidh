...
Source: https://modal.com/docs/guide/webhook-urls
...
This guide documents the behavior of URLs for [Web Functions](/docs/guide/webhooks) on Modal: automatic generation, configuration, programmatic retrieval, and more.

## Determine the Web Function URL from codeÂ

Modal Functions with the [`fastapi_endpoint`](/docs/sdk/py/latest/modal.fastapi_endpoint), [`asgi_app`](/docs/sdk/py/latest/modal.asgi_app), [`wsgi_app`](/docs/sdk/py/latest/modal.wsgi_app),
or [`web_server`](/docs/sdk/py/latest/modal.web_server) decorator
are made available over the Internet when they are [`serve`d](/docs/cli/latest/serve) or [`deploy`ed](/docs/cli/latest/deploy) and so they have a URL.

This URL is displayed in the `modal` CLI output
and is available in the Modal [dashboard](/apps) for the Function.

To determine a Functionâs URL programmatically,
check its [`get_web_url()`](/docs/sdk/py/latest/modal.Function#get_web_url) property:
...
You just need to do a [`from_name`](/docs/sdk/py/latest/modal.Function#from_name) based on the name of the Function and its [App](/docs/guide/apps):
...
just be the Workspace name. Multiple Environments are disambiguated by an [âEnvironment suffixâ](/docs/guide/environments#environment-web-suffixes), so
...
* *Source*:
  + *Workspace name slug*: `ECorp` â `ecorp`
  + *Environment web suffix slug*: `main` â `prod`
* *Label*:
  + *App name slug*: `text_to_speech` â `text-to-speech`
  + *Function name slug*: `flask_app` â `flask-app`

The full URL will be `https://ecorp-prod--text-to-speech-flask-app.modal.run`.

## User-specified labelsÂ

Itâs also possible to customize the `label` used for each Function
...
Youâll be asked to update your domain DNS records with your domain name
...
Custom domains can also be used with [ASGI](https://modal.com/docs/sdk/py/latest/modal.asgi_app#modalasgi_app) or [WSGI](https://modal.com/docs/sdk/py/latest/modal.wsgi_app) apps using the same `custom_domains` argument.

[Web Function URLs](#web-function-urls)[Determine the Web Function URL from code](#determine-the-web-function-url-from-code)[Auto-generated URLs](#auto-generated-urls)[User-specified labels](#user-specified-labels)[Ephemeral Apps](#ephemeral-apps)[Truncation](#truncation)[Custom domains](#custom-domains)