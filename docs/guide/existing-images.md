...
Source: https://modal.com/docs/guide/existing-images
...
## Load an image from a public registry with `.from_registry`Â
...
The `from_registry` method can load images from all public registries, such as [Nvidiaâs `nvcr.io`](https://catalog.ngc.nvidia.com/containers), [AWS ECR](https://aws.amazon.com/ecr/), and [GitHubâs `ghcr.io`](https://docs.github.com/en/packages/working-with-a-github-packages-registry/working-with-the-container-registry).
...
## Load images from private registriesÂ
...
### Docker Hub (Private)Â

To pull container images from private Docker Hub repositories, [create an access token](https://docs.docker.com/security/for-developers/access-tokens/) with âRead-Onlyâ permissions and use this token value and your Docker Hub
...
Use this Secret with the [`modal.Image.from_registry`](/docs/sdk/py/latest/modal.Image#from_registry) method.

### Elastic Container Registry (ECR)Â
...
As shown above, you also need to use a [Modal Secret](/docs/guide/secrets) containing the environment variables `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `AWS_REGION`. The AWS IAM user account associated
...
image layers at the time a new image is built. We donât store this token but
will cache the image once it has been pulled.

Images on ECR must be private and follow [image configuration requirements](/docs/sdk/py/latest/modal.Image#from_aws_ecr).

### Google Artifact Registry and Google Container RegistryÂ

For further detail on how to pull images from Googleâs image registries, see [`modal.Image.from_gcp_artifact_registry`](/docs/sdk/py/latest/modal.Image#from_gcp_artifact_registry).

### Azure Container Registry (ACR)Â

Modal doesnât have native Azure support, but you can pull images from a private ACR using
ACRâs [token-based repository permissions](https://learn.microsoft.com/en-us/azure/container-registry/container-registry-token-based-repository-permissions) to generate long-lived Docker credentials. Those credentials (token and password) can then be stored
as a Modal Secret and used with [`modal.Image.from_registry`](/docs/sdk/py/latest/modal.Image#from_registry) the same way as [Docker Hub private registry](#docker-hub-private) credentials.

## Bring your own image definition with `.from_dockerfile`Â

You can define an Image from an existing Dockerfile by passing its path to [`Image.from_dockerfile`](/docs/sdk/py/latest/modal.Image#from_dockerfile):
...
### Dockerfile command compatibilityÂ

Since Modal doesnât use Docker to build containers, we have our own
...
This is so the Modal Function runtimeâs Python entrypoint can run after your own. Most entrypoint
...
then you can register it as an entrypoint with `ENTRYPOINT ["/usr/bin/my_entrypoint.sh"]` in your Dockerfile, or with [`entrypoint`](/docs/sdk/py/latest/modal.Image#entrypoint) as an
Image build step.
...
We currently donât support default values in [interpolations](https://docs.docker.com/compose/compose-file/12-interpolation/),
such as `${VAR:-default}`

[Using existing images](#using-existing-images)[Load an image from a public registry with .from\_registry](#load-an-image-from-a-public-registry-with-from_registry)[Load images from private registries](#load-images-from-private-registries)[Docker Hub (Private)](#docker-hub-private)[Elastic Container Registry (ECR)](#elastic-container-registry-ecr)[Google Artifact Registry and Google Container Registry](#google-artifact-registry-and-google-container-registry)[Azure Container Registry (ACR)](#azure-container-registry-acr)[Bring your own image definition with .from\_dockerfile](#bring-your-own-image-definition-with-from_dockerfile)[Dockerfile command compatibility](#dockerfile-command-compatibility)[ENTRYPOINT](#entrypoint)[ENV](#env)
