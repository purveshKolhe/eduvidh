...
All deployed Modal Functions can be [invoked from any other Python application](/docs/guide/trigger-deployed-functions) using the Modal client library. We additionally provide multiple ways to expose
...
The easiest way to make a Python function addressable over the web uses the [`@modal.fastapi_endpoint` decorator](/docs/sdk/py/latest/modal.fastapi_endpoint):
...
This decorator wraps the Modal Function in a [FastAPI application](#how-do-web-functions-run-in-the-cloud).

*Note: Prior to v0.73.82, this function was named `@modal.web_endpoint`*.

### Developing with `modal serve`Â
...
Web Functions in other ways, for instance as [form data](https://fastapi.tiangolo.com/tutorial/request-forms/) and [file uploads](https://fastapi.tiangolo.com/tutorial/request-files/).

## How do Web Functions run in the cloud?Â
...
For the shortcut `@modal.fastapi_endpoint` decorator, Modal wraps your function in a [FastAPI](https://fastapi.tiangolo.com/) application. This means that the [Image](/docs/guide/images) your Function uses must have FastAPI installed, and the Functions that you write
need to follow its request and response [semantics](https://fastapi.tiangolo.com/tutorial). Web Functions can use
...
### ASGI apps - FastAPI, FastHTML, StarletteÂ

For ASGI apps, you can create a function decorated with [`@modal.asgi_app`](/docs/sdk/py/latest/modal.asgi_app) that returns a reference to
your web app:
...
event loops in ASGI applications. See [this guide](/docs/guide/concurrent-inputs) for details.

#### ASGI LifespanÂ

While we recommend using [`@modal.enter`](https://modal.com/docs/guide/lifecycle-functions#enter) for defining container lifecycle hooks, we also support the [ASGI lifespan protocol](https://asgi.readthedocs.io/en/latest/specs/lifespan.html). Lifespans begin when containers start, typically at the time of the first request. Hereâs an example using [FastAPI](https://fastapi.tiangolo.com/advanced/events/#lifespan):
...
### WSGI apps - Django, FlaskÂ

You can serve WSGI apps using the [`@modal.wsgi_app`](/docs/sdk/py/latest/modal.wsgi_app) decorator:
...
See [Flaskâs docs](https://flask.palletsprojects.com/en/2.1.x/deploying/asgi/) for more information on using Flask as a WSGI app.
...
For these cases, you can use the [`@modal.web_server`](/docs/sdk/py/latest/modal.web_server) decorator to âexposeâ a
...
See, for instance, our examples of how to serve [Streamlit](/docs/examples/serve_streamlit) and [vLLM](/docs/examples/vllm_inference) on Modal.

## Serve many configurations with parametrized functionsÂ

Python functions that launch ASGI/WSGI apps or web servers on Modal
cannot take arguments.

One simple pattern for allowing client-side configuration is to use [Parametrized Functions](/docs/guide/parametrized-functions). Each different
...
## Performance and scalingÂ

If you have no active containers when the Web Function receives a request, it will
experience a âcold startâ. Consult the guide page on [cold start performance](/docs/guide/cold-start) for more information on when
...
burst multiplier of 5 seconds. If you reach the rate limit, excess requests will return a [429 status code](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/429),
and youâll need to [get in touch](mailto:support@modal.com) with us about
raising the limit.
...
requests that arenât authorized to access your endpoint.

We also support conventional techniques for securing web servers.

### Token-based authenticationÂ

This is easy to implement in whichever framework youâre using. For example, if
youâre using `@modal.fastapi_endpoint` or `@modal.asgi_app` with FastAPI, you
...
This assumes you have a [Modal Secret](https://modal.com/secrets) named `my-web-auth-token` created, with contents `{AUTH_TOKEN: secret-random-token}`.
...
[Web Functions](#web-functions)[Simple endpoints](#simple-endpoints)[Developing with modal serve](#developing-with-modal-serve)[Deploying with modal deploy](#deploying-with-modal-deploy)[Passing arguments](#passing-arguments)[How do Web Functions run in the cloud?](#how-do-web-functions-run-in-the-cloud)[Serving ASGI and WSGI apps](#serving-asgi-and-wsgi-apps)[ASGI apps - FastAPI, FastHTML, Starlette](#asgi-apps---fastapi-fasthtml-starlette)[ASGI Lifespan](#asgi-lifespan)[WSGI apps - Django, Flask](#wsgi-apps---django-flask)[Non-ASGI web servers](#non-asgi-web-servers)[Serve many configurations with parametrized functions](#serve-many-configurations-with-parametrized-functions)[WebSockets](#websockets)[Performance and scaling](#performance-and-scaling)[Authentication](#authentication)[Token-based authentication](#token-based-authentication)[Client IP address](#client-ip-address)

Fully featured web apps

[LLM Voice Chat (React)](/docs/examples/llm-voice-chat)

[Stable Diffusion (Alpine)](/docs/examples/text_to_image)