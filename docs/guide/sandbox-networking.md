...
### Restricting by IP range (CIDR allowlist)Â
...
### Restricting by domain name (domain allowlist)Â
...
* **TLS (port 443)** connections are allowed only to the listed domains.
  ...
  the Sandboxâs system output stream.
...
### Updating the network policy at runtimeÂ

 

You can replace the outbound network policy of a running Sandbox without
restarting it. This is useful when an agentâs trust level changes mid-session â
...
* Each allowlist type must be set at creation time to be usable later. To
  ...
  with `outbound_domain_allowlist` (e.g. `["*"]`). The same applies to `outbound_cidr_allowlist` â create with `["0.0.0.0/0"]` if you want to
...
## Connecting to Sandboxes with HTTP and WebSocketsÂ
...
JSON-serialized metadata that was passed as `user_metadata` to `create_connect_token()`. This can be used by the application to
...
In some cases, itâs necessary to have a tunnel served through a custom domain
...
Other custom domains use `CNAME` forwarding. For tunnels, we need to use an `NS` record to delegate the domain to Modalâs nameservers.

**1. Delegate a (sub)domain to Modalâs nameservers.**

Add `NS` records to your DNS zone pointing to Modalâs nameservers. For example,
to use `sandbox.example.com`, add the following records in your DNS providerâs
control panel:
...
Reach out to us on Slack and provide the domain name. Weâll enable it for your
workspace.
...
[Networking and security](#networking-and-security)[Outbound access control](#outbound-access-control)[Blocking all network access](#blocking-all-network-access)[Restricting by IP range (CIDR allowlist)](#restricting-by-ip-range-cidr-allowlist)[Restricting by domain name (domain allowlist)](#restricting-by-domain-name-domain-allowlist)[Updating the network policy at runtime](#updating-the-network-policy-at-runtime)[Dynamic policy limitations](#dynamic-policy-limitations)[Inbound access control](#inbound-access-control)[Connecting to Sandboxes with HTTP and WebSockets](#connecting-to-sandboxes-with-http-and-websockets)[Forwarding ports](#forwarding-ports)[Custom domains](#custom-domains)[Security model](#security-model)
