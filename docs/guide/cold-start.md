...
1. inputs may **spend more time waiting** in a queue for a container
   to become ready or âwarmâ.
2. when an input is handled by the container that just started,
   there may be **extra work that only needs to be done on the first invocation** (âinitializationâ).

If you are invoking Functions with no warm containers
or if you otherwise see inputs spending too much time in the âpendingâ state,
...
## Reduce time spent queueing for warm containersÂ
...
### Warm up containers fasterÂ

The time taken for a container to become warm
and ready for inputs can range from seconds to minutes.

Modalâs custom container stack has been heavily optimized to reduce this time.
...
we need to execute any logic in your codeâs global scope (such as imports)
...
#### Keep containers warm for longer with `scaledown_window`Â
...
the `scaledown_window` on the [`@function`](/docs/sdk/py/latest/modal.App#function) decorator. The value is measured in seconds, and it can be set anywhere between
...
#### Overprovision resources with `min_containers` and `buffer_containers`Â

Keeping already warm containers around longer doesnât help if there are no warm
containers to begin with, as when Functions scale from zero.

To keep some containers warm and running at all times, set the `min_containers` value on the [`@function`](/docs/sdk/py/latest/modal.App#function) decorator. This
puts a floor on the the number of containers so that the Function doesnât scale
...
This âbufferâ of extra containers will be idle and ready to handle inputs if
...
## Reduce latency from initializationÂ
...
### Move initialization work out of the first invocationÂ
...
should be done as early as possible. The results can be included in the [containerâs Image](/docs/guide/images) or saved to a [Modal Volume](/docs/guide/volumes).
...
Note that `enter` doesnât get rid of the latency â
...
### Share initialization work across cold starts with Memory SnapshotsÂ

Cold starts can also be made faster by using Modal [Memory Snapshots](/docs/guide/memory-snapshots).
...
Memory snapshotting captures the state of a containerâs memory
...
Refer to the [Memory Snapshots guide](/docs/guide/memory-snapshots) for details.

### Optimize initialization codeÂ
...
#### Load multiple large files concurrentlyÂ
...
full advantage of our platformâs high disk and network bandwidth
to reduce latency.
...
starts, itâs possible that some part of your functionâs code is holding the
Python [GIL](https://wiki.python.org/moin/GlobalInterpreterLock) and reducing
the efficacy of the multi-threaded executor.

[Cold start performance](#cold-start-performance)[What is a cold start?](#what-is-a-cold-start)[Reduce time spent queueing for warm containers](#reduce-time-spent-queueing-for-warm-containers)[Warm up containers faster](#warm-up-containers-faster)[Run more warm containers](#run-more-warm-containers)[Keep containers warm for longer with scaledown\_window](#keep-containers-warm-for-longer-with-scaledown_window)[Overprovision resources with min\_containers and buffer\_containers](#overprovision-resources-with-min_containers-and-buffer_containers)[Reduce latency from initialization](#reduce-latency-from-initialization)[Move initialization work out of the first invocation](#move-initialization-work-out-of-the-first-invocation)[Share initialization work across cold starts with Memory Snapshots](#share-initialization-work-across-cold-starts-with-memory-snapshots)[Optimize initialization code](#optimize-initialization-code)[Load multiple large files concurrently](#load-multiple-large-files-concurrently)