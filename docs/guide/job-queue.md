...
2. Submitting a job using [`modal.Function.spawn()`](/docs/sdk/py/latest/modal.Function#spawn)
3. Polling for the jobâs result using [`modal.FunctionCall.get()`](/docs/sdk/py/latest/modal.FunctionCall#get)

Hereâs a simple example that you can run with `modal run my_job_queue.py`:
...
* `get_job_result` attempts to retrieve the result of a previously submitted job
  using [`FunctionCall.from_id()`](/docs/sdk/py/latest/modal.FunctionCall#from_id) and [`FunctionCall.get()`](/docs/sdk/py/latest/modal.FunctionCall#get). [`FunctionCall.get()`](/docs/sdk/py/latest/modal.FunctionCall#get) waits indefinitely
  ...
  immediately. Here, if the job hasnât completed yet, we return a pending
  response.
...
Hereâs an example, assuming that you have already deployed `process_job` on
Modal with `modal deploy` as above. This example wonât work if you havenât
...
* The `/submit` endpoint accepts job data, submits a new job using `await process_job.spawn.aio()`, and returns the jobâs ID to the client.
* The `/result/{call_id}` endpoint allows the client to poll for the jobâs
  result using the job ID. If the job hasnât completed yet, it returns a 202
...
You can customize the behavior of the job queue by configuring the `@app.function()` decorator with options like [`retries`](/docs/guide/retries#function-retries), [`timeout`](/docs/guide/timeouts#timeouts), and [`max_containers`](/docs/guide/scale#configuring-autoscaling-behavior).
