...
The timeout duration is a measure of a Functionâs *execution* time. It does not
...
### Container startup timeoutÂ

A Functionâs `startup_timeout` configures the containerâs *startup* time. Your container
may be taking a long time to startup because it is loading large data, initializing a
large model or importing many packages. In these cases, you can extend the `startup_timeout` of your Function.
...
`startup_timeout` was added in v1.1.4. Prior to v1.1.4, `timeout` configures the *execution* time and *startup* time. If `startup_timeout` is not set, `timeout` will
...
[Timeouts](#timeouts)[Container startup timeout](#container-startup-timeout)[Handling timeouts](#handling-timeouts)[Timeout accuracy](#timeout-accuracy)