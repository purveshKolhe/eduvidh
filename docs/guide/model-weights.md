...
which acts as a distributed file system, a âshared diskâ all of your Modal Functions can access.

## Storing weights in a Modal VolumeÂ
...
### Saving model weights into a Modal Volume from a Modal FunctionÂ
...
### Uploading model weights into a Modal VolumeÂ
...
For that, you can use the `batch_upload` method of [`modal.Volume`](/docs/sdk/py/latest/modal.Volume)s
...
Alternatively, you can upload model weights using the [`modal volume`](/docs/cli/latest/volume) CLI command:
...
### Mounting cloud buckets as Modal VolumesÂ
...
## Reading model weights from a Modal VolumeÂ
...
## Storing weights in the Modal ImageÂ

It is also possible to store weights in your Functionâs Modal [Image](/docs/guide/images),
the private file system state that a Function sees when it starts up.
The weights might be downloaded via shell commands with [`Image.run_commands`](/docs/guide/images) or downloaded using a Python function with [`Image.run_function`](/docs/guide/images).
...
## Optimizing model weight reads with `@modal.enter`Â

In the above code samples, weights are loaded from disk into memory each time
the `inference` function is run. This isnât so bad if inference is much
...
To guarantee a particular modelâs weights are only loaded once, you can use the `@modal.enter` [container lifecycle hook](/docs/guide/lifecycle-functions) to load the weights only when a new container starts.
...
Note that methods decorated with `@modal.enter` canât be passed dynamic arguments.

If you need to load a single but possibly different model on each container start, you can [parametrize](/docs/guide/parametrized-functions) your Modal Cls.
...
## Storing weights from the Hugging Face Hub on ModalÂ
