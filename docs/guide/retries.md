...
## Automatically recover from flakes with `retries`Â
...
## Handle failures in `Function.map`Â
...
[Failures and retries](#failures-and-retries)[Automatically recover from flakes with retries](#automatically-recover-from-flakes-with-retries)[Handle failures in Function.map](#handle-failures-in-functionmap)[Container crashes](#container-crashes)