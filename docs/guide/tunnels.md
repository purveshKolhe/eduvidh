...
You can also create tunnels on a [Sandbox](/docs/guide/sandbox-networking#forwarding-ports) to directly expose the containerâs ports.
...
When you run the function, it starts Jupyter and gives you the public URL. Itâs
as simple as that.

All Modal features are supported. If you [need GPUs](https://modal.com/docs/guide/gpu), pass `gpu=` to the `@app.function()` decorator. If you [need more CPUs, RAM](https://modal.com/docs/guide/resources), or to attach [volumes](https://modal.com/docs/guide/volumes), those
also just work.

### Programmable startupÂ
...
### Advanced: Unencrypted TCP tunnelsÂ

By default, tunnels are only exposed to the Internet at a secure random URL, and
connections have automatic TLS (the âSâ in HTTPS). However, sometimes you might
...
Modal only charges for containers based on [the resources you use](https://modal.com/pricing). There is no additional
...
Tunnels are run on Modalâs private global network of Internet relays. On
...
Youâre just getting the TLS-encrypted TCP stream directly!

[Tunnels](#tunnels)[Build with tunnels](#build-with-tunnels)[Programmable startup](#programmable-startup)[Advanced: Unencrypted TCP tunnels](#advanced-unencrypted-tcp-tunnels)[Pricing](#pricing)[Security](#security)