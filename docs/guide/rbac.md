...
Source: https://modal.com/docs/guide/rbac
...
Modalâs RBAC system operates at two levels:
...
* **Owner** â Full read-write access to everything in the Workspace, including billing, Workspace management, and all Environments. Can assign any Role to other members.
* **Manager** â Same as Owner, but cannot modify the Owner Role.
* **Member** â Can deploy and manage Apps, but cannot access billing, Workspace management, or other Workspace settings.

## Environment RolesÂ

Modal [Environments](/docs/guide/environments) isolate Modal Apps and other resources from one another within a Workspace.

Every Environment has two Environment Roles that determine access to it:

* **Viewer** â Read-only access to resources in the Environment, including dashboards, logs, metrics, app and function configuration.
* **Contributor** â Full read and write access to the Environment. Workspace Owners and Managers always have Contributor access.

By default, every user and service user has **Contributor** access to an Environment. Setting an Environment as **Restricted** lets you change their Roles.

## Setting up restricted EnvironmentsÂ
...
2. **Create or restrict an Environment**: To create a new restricted Environment, use [`modal environment create --restricted NAME`](/docs/cli/latest/environment#modal-environment-create). To restrict an existing unrestricted Environment, navigate to your Workspaceâs
...
## Service users and service tokensÂ
...
| --- | --- | --- |
| Interactive development or manual management | Human user | Access is based on the userâs Workspace Role, plus any Environment Role for restricted Environments |
| Automation in CI/CD or deployment workflows | Service user authenticated with a service token | Access is based only on the service userâs Environment Role |
...
## Proxy tokens for Web FunctionsÂ
...
On workspaces with RBAC enabled, proxy tokens are **scoped** â each token is explicitly associated with one or more Environments, and will only be accepted for Web Functions deployed to those Environments. This prevents a token intended for a staging endpoint from being used to call a production one.

### Creating a scoped proxy tokenÂ

1. Navigate to **Settings â Proxy Tokens** and click **New Token**.
2. Copy the token ID and secret â the secret is only shown once.
...
### Scoped vs. workspace-wide tokensÂ
...
### Cross-Environment behavior for app and task identitiesÂ
...
### Example: inbound vs. outbound accessÂ
...
## Protecting production secrets with restricted EnvironmentsÂ
...
[Role-Based Access Control (RBAC)](#role-based-access-control-rbac)[Workspace Roles](#workspace-roles)[Environment Roles](#environment-roles)[Setting up restricted Environments](#setting-up-restricted-environments)[Default access by actor](#default-access-by-actor)[Service users and service tokens](#service-users-and-service-tokens)[Proxy tokens for Web Functions](#proxy-tokens-for-web-functions)[Creating a scoped proxy token](#creating-a-scoped-proxy-token)[Scoped vs. workspace-wide tokens](#scoped-vs-workspace-wide-tokens)[Cross-Environment access](#cross-environment-access)[Cross-Environment behavior for app and task identities](#cross-environment-behavior-for-app-and-task-identities)[Example: inbound vs. outbound access](#example-inbound-vs-outbound-access)[Protecting production secrets with restricted Environments](#protecting-production-secrets-with-restricted-environments)[Common access patterns](#common-access-patterns)[FAQ](#faq)