...
Source: https://modal.com/docs/guide/endpoint-benchmarks
...
repeatable load â so you can compare models, regions, and configurations on an
apples-to-apples basis.

Modal runs benchmarks for you: it drives a standard load generator against your
live endpoint from a sandbox and reports the results. Start one from the **Benchmark** tab on the endpointâs detail page. The resulting metrics are
...
| --- | --- | --- |
| **Real-time generation** | ~3,000 input tokens â ~100 output tokens, randomized prompts | Interactive chat / short-answer Q&A |
| **Agentic multi-turn** | ~45,000-token shared system prefix + ~5,000-token question â ~200 output tokens | Agent / tool-use workloads with long, reused context |

The agentic pattern reuses a long shared prefix across requests, so it also
exercises prefix caching â a major factor in agent workload performance.

## Endpoint preview benchmarksÂ

When you pick a model while creating an endpoint, youâll also see **precomputed
benchmarks** attached to the modelâs recipe. These are reference numbers Modal
...
theyâre produced ahead of time by Modal (not run against your endpoint), and
theyâre tied to the recipe rather than your specific deployment. Use recipe
...
[Benchmark an endpoint](#benchmark-an-endpoint)[Workload patterns](#workload-patterns)[Endpoint preview benchmarks](#endpoint-preview-benchmarks)[Caveats](#caveats)