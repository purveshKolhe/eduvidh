...
Source: https://modal.com/docs/guide/custom-container
...
For reference documentation on the `modal.Image` object, see [this page](/docs/sdk/py/latest/modal.Image).

## What are Images?Â

Your code on Modal runs in *containers*. Containers are like light-weight
virtual machines â container engines use [operating system tricks](https://earthly.dev/blog/chroot/) to isolate programs
from each other (âcontainingâ them), making them work as though they were
...
Containers are started up from a stored âsnapshotâ of their filesystem state
called an *image*. Producing the image for a container is called *building* the
image.

By default, Modal Functions and Sandboxes run in a [Debian Linux](https://en.wikipedia.org/wiki/Debian) container with a basic
...
features like `RUN` and `ENV`. Weâll cover each of these in this guide,
along with tips and tricks for building Images effectively when using each tool.

## Add Python packagesÂ
...
If you run into any issues with [`Image.uv_pip_install`](/docs/sdk/py/latest/modal.Image#uv_pip_install), then
...
function if you so choose, you donât need to worry about virtual
...
matching the one youâre running locally, provide the `python_version` as a
string when constructing the base image, like we did above.

## Add local files with `add_local_dir` and `add_local_file`Â

Sometimes your containers need a dependency thatâs not available on the Internet,
...
also means you canât run additional build steps after. You can specify a `copy=True` argument
to the `add_local_` methods to instead force the files to be included in the built Image.

### Add local Python code with `add_local_python_source`Â

You can add Python code thatâs importable locally to your container
by providing the module name to [`Image.add_local_python_source`](/docs/sdk/py/latest/modal.Image#add_local_python_source).
...
instead of a file system path and looks up the local packageâs or moduleâs location via Pythonâs importing
...
Third party packages should be installed via [`Image.uv_pip_install`](/docs/sdk/py/latest/modal.Image#uv_pip_install) or similar.

### What if I have different Python packages locally and remotely?Â

You might want to use packages inside your Modal code that you donât have on
your local computer. In the example above, we build a container that uses `pandas`. But if we donât have `pandas` locally, on the computer building the
Modal App, we canât put `import pandas` at the top of the script, since it would
...
objects, like `pandas.DataFrame`s, and if your local machine doesnât have `pandas` installed, it wonât be able to handle a `pandas` object (the error
...
imports. In that case, you can use the [`Image.imports`](/docs/sdk/py/latest/modal.Image#imports) context manager:
...
you can combine this context manager with [Memory Snapshots](/docs/guide/memory-snapshots) to improve [cold start performance](/docs/guide/cold-start#share-initialization-work-across-cold-starts-with-memory-snapshots) for Functions that frequently scale up.

## Install system packages with `.apt_install`Â

You can install Linux packages with the [`apt` package manager](https://www.debian.org/doc/manuals/apt-guide/index.en.html) using [`Image.apt_install`](/docs/sdk/py/latest/modal.Image#apt_install):
...
## Set environment variables with `.env`Â
...
## Run shell commands with `.run_commands`Â
...
## Run a Python function during your build with `.run_function`Â

You can run Python code as a build step using the [`Image.run_function`](/docs/sdk/py/latest/modal.Image#run_function) method.
...
For details on storing model weights on Modal, see [this guide](/docs/guide/model-weights).

Essentially, this is equivalent to running a Modal Function and snapshotting the
resulting filesystem as a new Image. Any kwargs accepted by [`@app.function`](/docs/sdk/py/latest/modal.App#function) ([`Volume`s](/docs/guide/volumes), [`Secret`s](/docs/guide/secrets), specifications of
...
functions. See the [reference documentation](/docs/sdk/py/latest/modal.Image#run_function) for details.

## Attach GPUs during setupÂ
...
## Use `mamba` instead of `pip` with `micromamba_install`Â
...
## Image caching and rebuildsÂ

Modal uses the definition of an Image to determine whether it needs to be
rebuilt. If the definition hasnât changed since the last time you ran or
...
definition hasnât changed. You can do this by adding the `force_build=True` argument to any of the Image building methods.
...
As in other cases where a layerâs definition changes, both the `pip_install` and `run_commands` layers will rebuild, but the `apt_install` will not. Remember to
remove `force_build=True` after youâve rebuilt the Image, or it will
...
any App that uses that base. If youâre debugging an issue with your Image, a better
...
causing unpredictable rebuilds: the âImage Builder Versionâ. This is a workspace
...
your workspaceâs configuration when convenient. After updating, your next
...
You can set the Image Builder Version for your workspace by going to your [workspace settings](/settings/image-builder-version). This page also documents the
important updates in each version.

[Images](#images)[What are Images?](#what-are-images)[Add Python packages](#add-python-packages)[Add local files with add\_local\_dir and add\_local\_file](#add-local-files-with-add_local_dir-and-add_local_file)[Add local Python code with add\_local\_python\_source](#add-local-python-code-with-add_local_python_source)[What if I have different Python packages locally and remotely?](#what-if-i-have-different-python-packages-locally-and-remotely)[Install system packages with .apt\_install](#install-system-packages-with-apt_install)[Set environment variables with .env](#set-environment-variables-with-env)[Run shell commands with .run\_commands](#run-shell-commands-with-run_commands)[Run a Python function during your build with .run\_function](#run-a-python-function-during-your-build-with-run_function)[Attach GPUs during setup](#attach-gpus-during-setup)[Use mamba instead of pip with micromamba\_install](#use-mamba-instead-of-pip-with-micromamba_install)[Image caching and rebuilds](#image-caching-and-rebuilds)[Image builder updates](#image-builder-updates)

See it in action

[Define an Image for LLM inference](/docs/examples/llm_inference)

[Build Images that use CUDA](/docs/examples/install_cuda)