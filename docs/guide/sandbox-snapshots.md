...
Source: https://modal.com/docs/guide/sandbox-snapshots
...
Sandboxes support snapshotting, allowing you to save your Sandboxâs state
and restore it later. This is useful for:

* Reducing startup latency
* Creating custom environments for your Sandboxes to run in
* Backing up your Sandboxâs state for debugging
* Running large-scale experiments with the same initial state
* Branching your Sandboxâs state to test different code changes independently
...
If you try to use an expired snapshot, Modal will raise a `NotFoundError` â immediately when mounting the Image into a running Sandbox, or upon first interaction (e.g. `exec` or `wait`) when starting a new Sandbox from the expired Image. Note that `Image.from_id()` is itself lazy and will not raise an error on construction even if the provided Image ID has been deleted.
...
## Filesystem SnapshotsÂ

Filesystem Snapshots are copies of the Sandboxâs filesystem at a given point in time.
These Snapshots are [Images](/docs/sdk/py/latest/modal.Image) and can be used to create
new Sandboxes.

To create a Filesystem Snapshot, you can use the [`Sandbox.snapshot_filesystem()`](/docs/sdk/py/latest/modal.Sandbox#snapshot_filesystem) method:
...
Use `snapshot_directory` to snapshot a directory, `mount_image` to mount a previous directory snapshot at a directory path,
and `unmount_image` to remove that mounted Image later.
To protect directory snapshots with customer-held key material, see [Customer Supplied Encryption Keys](/docs/guide/customer-supplied-encryption-keys#directory-snapshots).
...
Sandbox memory snapshots are copies of a Sandboxâs entire state, both in memory and on the filesystem. These Snapshots can be restored later to create a new Sandbox, which is an exact clone of the original Sandbox.
...
This means a âchainâ of snapshotted state can only ever become as old as the expiration date of the first snapshot in the series.
...
* Sandbox memory snapshots can only be restored on the same exact instance type that the original Sandbox was run on. Given Modalâs diverse fleet of capacity, this can sometimes lead to scheduling delays, especially when memory snapshots are combined with narrow region pinning.

## Persisting Sandbox StateÂ
...
[Snapshots](#snapshots)[Snapshot Retention](#snapshot-retention)[Filesystem Snapshots](#filesystem-snapshots)[Directory Snapshots](#directory-snapshots)[Usage](#usage)[Unmounting a mounted Image](#unmounting-a-mounted-image)[Memory Snapshots](#memory-snapshots)[Re-snapshotting](#re-snapshotting)[Limitations](#limitations)[Persisting Sandbox State](#persisting-sandbox-state)[Deleting Snapshots](#deleting-snapshots)