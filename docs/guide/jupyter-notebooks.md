...
Source: https://modal.com/docs/guide/jupyter-notebooks
...
## Modal inside JupyterÂ

You can use the Modal client library in notebook environments like Jupyter! Just `import modal` and use as normal. You will likely need to use [`app.run`](/docs/guide/apps#ephemeral-apps) to create an ephemeral App to run your Functions:
...
That will start a Jupyter instance with an A10G GPU attached. Youâll be able to
access the app with via a [Modal Tunnel URL](https://modal.com/docs/guide/tunnels). Jupyter
...
[Jupyter notebooks](#jupyter-notebooks)[Modal inside Jupyter](#modal-inside-jupyter)[Known issues](#known-issues)[Jupyter inside Modal](#jupyter-inside-modal)[Further examples](#further-examples)