...
Source: https://modal.com/docs/guide/cuda
...
see [this guide](/docs/guide/gpu). For a deep dive on both the [GPU hardware](/gpu-glossary/device-hardware) and [software](/gpu-glossary/device-software) and for even more detail on [the CUDA stack](/gpu-glossary/host-software/),
...
When someone refers to âinstalling CUDAâ or âusing CUDAâ,
they are referring not to a library, but to a [stack](/gpu-glossary/host-software/cuda-software-platform) with multiple layers.
...
### Level 0: Kernel-mode driver componentsÂ
...
These components communicate directly with hardware â in this case the GPU.
...
### Level 1: User-mode driver APIÂ

All action in Linux that doesnât occur in the kernel occurs in [user space](https://en.wikipedia.org/wiki/User_space).
...
* the [NVIDIA management library](https://developer.nvidia.com/management-library-nvml), `libnvidia-ml.so`, and its command line interface [`nvidia-smi`](https://developer.nvidia.com/system-management-interface).
  You can use these tools to check the status of the systemâs GPU(s).
...
So why arenât these components installed along with the drivers?
...
## Install GPU-accelerated `torch` and `transformers` with `pip_install`Â
...
## For more complex setups, use an officially-supported CUDA imageÂ
...
Older versions in the `12.*` and `13.*` series are guaranteed to be compatible with the host machineâs driver,
...
[Run Stable Diffusion with a CLI, API, and web UI](/docs/examples/text_to_image)