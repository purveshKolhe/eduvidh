...
## Improvements over gVisor sandboxesÂ
...
The following limitations are known and weâre tracking them:

* **GPUs are not supported.** VM Sandboxes currently only support CPU workloads.
* **The [Sandbox filesystem API](/docs/guide/sandbox-files#filesystem-api-beta) is only available in new SDK versions**. For the Python SDK, it requires version â¥ 1.4.0 and for the JS/TS/Go SDKs, it requires versions â¥ 0.7.6.
...
* **[Memory Snapshots](/docs/guide/sandbox-snapshots#memory-snapshots) are not yet supported.** Only [Filesystem Snapshots](/docs/guide/sandbox-snapshots#filesystem-snapshots) work on VM Sandboxes today.
* **Root images â¥ 512 GiB are not supported.** The VM root filesystem is currently limited to 512 GiB. Sandboxes created from container images exceeding this size will fail to start.

If you hit a rough edge that isnât listed here, please reach out via [Slack](/slack) or email us at [support@modal.com](mailto:support@modal.com).

[VM Sandboxes](#vm-sandboxes)[Improvements over gVisor sandboxes](#improvements-over-gvisor-sandboxes)[Resource model](#resource-model)[Limitations](#limitations)