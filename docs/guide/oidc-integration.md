...
Source: https://modal.com/docs/guide/oidc-integration
...
Generated tokens are injected into your Functionâs containers via the `MODAL_IDENTITY_TOKEN` environment variable. Below is an example of what claims might be included in a token:
...
## Demo usage with AWS S3Â

To see how OIDC tokens can be used, weâll demo a simple Function that lists
objects in an S3 bucket.

### Step 0: Understand your OIDC claimsÂ
...
### Step 1: Configure AWS to trust Modalâs OIDC providerÂ

We need to make AWS accept Modal identity tokens. To do this, we need to add
Modalâs OIDC provider as a trusted entity in our AWS account.
...
### Step 2: Create an IAM policy that can be used by Modal FunctionsÂ

Letâs create a simple IAM policy that allows listing objects in an S3 bucket.
...
### Step 3: Create an IAM role that can be assumed by Modal FunctionsÂ
...
### Step 4: Use the OIDC token in your FunctionÂ
...
## Demo usage with AWS Elastic Container Registry (ECR)Â

You can also use OIDC to authenticate [Private Registries](/docs/guide/existing-images) on AWS.

### PrerequisitesÂ

1. Configure AWS to trust Modalâs OIDC provider ([Step 1 above](#step-1-configure-aws-to-trust-modals-oidc-provider))
...
[Using OIDC to authenticate with external services](#using-oidc-to-authenticate-with-external-services)[How it works](#how-it-works)[App name format](#app-name-format)[Demo usage with AWS S3](#demo-usage-with-aws-s3)[Step 0: Understand your OIDC claims](#step-0-understand-your-oidc-claims)[Step 1: Configure AWS to trust Modalâs OIDC provider](#step-1-configure-aws-to-trust-modals-oidc-provider)[Step 2: Create an IAM policy that can be used by Modal Functions](#step-2-create-an-iam-policy-that-can-be-used-by-modal-functions)[Step 3: Create an IAM role that can be assumed by Modal Functions](#step-3-create-an-iam-role-that-can-be-assumed-by-modal-functions)[Step 4: Use the OIDC token in your Function](#step-4-use-the-oidc-token-in-your-function)[Demo usage with AWS Elastic Container Registry (ECR)](#demo-usage-with-aws-elastic-container-registry-ecr)[Prerequisites](#prerequisites)[Test with a sample image](#test-with-a-sample-image)[Next steps](#next-steps)