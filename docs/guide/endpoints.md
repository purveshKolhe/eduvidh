...
Source: https://modal.com/docs/guide/endpoints
...
Deploy a production-ready LLM inference endpoint on Modalâs managed
...
* **Fast inference by default** â every endpoint runs behind a low-latency
  request proxy on tuned open-source inference engines, with SOTA speculative
  decoding wherever the recipe supports it.
* **Usage-based pricing** â you pay only for the *compute* your endpoint uses,
  so you reap the benefits of our compute engine optimizations.
* **Scale-to-zero autoscaling** â endpoints scale up under load and down to zero
...
come online. You can also create endpoints from the [**Endpoints**](https://modal.com/endpoints) tab in the dashboard â the form
collects the same options.

If you omit the `name` argument, Modal derives one from the model
(`Qwen/Qwen3.5-4B` â `qwen3-5-4b`).
...
and canât be retrieved later, so store it somewhere safe:
...
If your workspace has [RBAC](/docs/guide/rbac) enabled, youâll also need to explicitly
associate the new token with the environment where youâll create the endpoint:

```
modal workspace proxy-tokens allow wk-... main
```

You can also make requests to an authenticated endpoint using the [`modal curl`](/docs/cli/latest/curl) utility. This performs transparent
...
## Calling your endpointÂ

Once the endpoint is live, it serves the OpenAI Chat Completions API at the
endpoint URL â find it in the dashboard or with `modal endpoint list`. The API
...
List the models the endpoint serves with a `GET` request, passing your [proxy token](#proxy-tokens) in the `Modal-Key` and `Modal-Secret` headers:
...
## Choosing where it runsÂ

Two placement controls:

* **Routing region** (`--routing-region`) â where the request proxy is anchored.
  Pick the region closest to your callers: `us-west` (default), `us-east`, `eu-west`, or `ap-south`.
* **Compute placement** (`--colocate-compute`) â by default Modal places
...
underlying code by navigating to the âSourceâ panel in the endpoint dashboard.
...
overrides in the UI. Region pinning applies a [region selection multiplier](https://modal.com/pricing).

[Endpoints](#endpoints)[Getting started](#getting-started)[Proxy tokens](#proxy-tokens)[Calling your endpoint](#calling-your-endpoint)[Serving custom weights](#serving-custom-weights)[Choosing where it runs](#choosing-where-it-runs)[Managing endpoints](#managing-endpoints)[Viewing the source](#viewing-the-source)[Pricing](#pricing)