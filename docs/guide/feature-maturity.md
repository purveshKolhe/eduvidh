...
* [Release phases](#release-phases) ([Alpha](#alpha), [Beta](#beta) or [GA](#general-availability-ga)): signals the stability of a featureâs underlying infrastructure
...
We use the following release phases to signal the maturity of a featureâs underlying design and infrastructure:
...
This is strictly an SDK concept which indicates API stability, not infrastructure maturity. It often correlates with the Alpha â Beta â GA progression, but not always. Some features stabilize their API early while the backend is still maturing, but experimental APIs may also be introduced later in a featureâs lifecycle to provide additional depth of configuration.

An experimental tag means weâre still gathering feedback and iterating on the interface: method names, parameters, or return types may change. Once weâre confident in the design, we remove the experimental marker and commit to backwards compatibility.
...
We value your feedback on Alpha and Beta features! If youâre using a feature at any release phase and have suggestions or encounter issues:

* Join our [Slack community](https://modal.com/slack) to discuss with the team and other users
* Reach out to [support@modal.com](mailto:support@modal.com) with specific feedback or bug reports

[Feature Maturity](#feature-maturity)[Release Phases](#release-phases)[Alpha](#alpha)[Beta](#beta)[General Availability (GA)](#general-availability-ga)[Experimental SDK](#experimental-sdk)[SDK Deprecations](#sdk-deprecations)[Other interfaces](#other-interfaces)[Providing Feedback](#providing-feedback)