...
Source: https://modal.com/docs/guide/managing-deployments
...
Once youâve finished using `modal run` or `modal serve` to iterate on your Modal
code, itâs time to deploy. A Modal deployment creates and then persists an
App and its objects, providing the following benefits:

* Repeated executions of the Appâs Functions will be grouped under the Deployment,
...
* Function calls are much faster because deployed Functions are persistent and
  ...
  Functions in [Invoking deployed Functions](/docs/guide/trigger-deployed-functions).
...
## Creating deploymentsÂ

Deployments are created using the [`modal deploy`](/docs/cli/latest/deploy) command.

```
 % modal deploy -m whisper_pod_transcriber.main
â Initialized. View app page at https://modal.com/apps/ap-PYc2Tb7JrkskFUI8U5w0KG.
â Created objects.
âââ ð¨ Created populate_podcast_metadata.
âââ ð¨ Mounted /home/ubuntu/whisper_pod_transcriber at /root/whisper_pod_transcriber
âââ ð¨ Created fastapi_app => https://modal-labs-whisper-pod-transcriber-fastapi-app.modal.run
âââ ð¨ Mounted /home/ubuntu/whisper_pod_transcriber/whisper_frontend/dist at /assets
âââ ð¨ Created search_podcast.
âââ ð¨ Created refresh_index.
âââ ð¨ Created transcribe_segment.
âââ ð¨ Created transcribe_episode..
âââ ð¨ Created fetch_episodes.
â App deployed! ð
...
Deployments can also be created programmatically using the [`app.deploy()`](/docs/sdk/py/latest/modal.App#deploy) method in Modalâs Python SDK.

## Viewing deploymentsÂ

Deployments can be viewed in the [web UI](/apps) on an Appâs âDeployment Historyâ
page, or from the command line using the [`modal app list`](/docs/cli/latest/app#modal-app-list) command.

### Deployment events on chartsÂ

You can overlay deployment history information on your Functionâs metric charts by enabling
the **Show Deployments** toggle. Each marker represents one or more deployments
that occurred within a time bucket.

Hovering over a marker shows the version number and timestamp of each deployment, plus a link to the full âDeployment Historyâ page.

![Deployment history overlay on a metric chart](https://modal-cdn.com/cdnbot/deployment-historyt991cvw__b284b7fa.webp)

## Updating deploymentsÂ

A deployment can create a new App or redeploy an existing deployed App with
a new version. Itâs useful to understand how Modal handles the transition between
...
deployment strategy, configured with `--strategy` in the [`modal deploy`](/docs/cli/latest/deploy) CLI or `strategy=` in the [`app.deploy()`](/docs/sdk/py/latest/modal.App#deploy) method.
...
## No-op deployments and rolloversÂ
...
Itâs possible to cycle the containers serving an App without any changes to the
code or configuration by using the [`modal app rollover`](/docs/cli/latest/app#modal-app-rollover) command. This may be
...
or using the [`modal app rollback`](/docs/cli/latest/app#modal-app-rollback) CLI. Rollback deployments look like new deployments: they increment the version
number and are attributed to the user who triggered the rollback. But the Appâs
Functions and metadata will be reset to their previous state independently of
your current App codebase.

## Stopping deploymentsÂ

Deployed Apps can be stopped in the web UI by clicking the red âStop appâ button on
the Appâs âOverviewâ page, or alternatively from the command line using the [`modal app stop`](/docs/cli/latest/app#modal-app-stop) command.
...
[Managing deployments](#managing-deployments)[Creating deployments](#creating-deployments)[Viewing deployments](#viewing-deployments)[Deployment events on charts](#deployment-events-on-charts)[Updating deployments](#updating-deployments)[Deployment strategies](#deployment-strategies)[No-op deployments and rollovers](#no-op-deployments-and-rollovers)[Deployment rollbacks](#deployment-rollbacks)[Stopping deployments](#stopping-deployments)