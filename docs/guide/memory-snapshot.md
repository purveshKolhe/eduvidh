...
In our experience, practical initialization-heavy Functions often start up [3-10x faster from Memory Snapshots](/blog/gpu-mem-snapshots).

There are two variants of Memory Snapshots. [CPU Memory Snapshots](#cpu-memory-snapshots) capture the state of CPU memory. [GPU Memory Snapshots](#gpu-memory-snapshots), an [alpha feature](/docs/guide/feature-maturity), also capture the state of GPU memory.

## CPU Memory SnapshotsÂ
...
Use the [`Image.imports` context manager](/docs/sdk/py/latest/modal.Image#imports) to import remote-only dependencies in the global scope.
...
## Container lifecycle hooks and Memory SnapshotsÂ

Modalâs [container lifecycle hooks](/docs/guide/lifecycle-functions) provide additional control over what parts of container initialization work
...
Youâll generally want to include any expensive initialization work that
requires the GPU in the Memory Snapshot.
Use a Modal [Cls](/docs/guide/lifecycle-functions) and put that work inside a `@modal.enter` method,
like so:
...
### Limitations of GPU Memory SnapshotsÂ

[Weâve seen](/blog/gpu-mem-snapshots) that GPU Memory Snapshots can massively reduce cold start time,
...
#### You may need to rewrite code for compatibility or to improve performanceÂ
...
then recreate it on restore, rather than writing and then reading the KV cacheâs meaningless pages in a snapshot.
See [this example with vLLM](/docs/examples/vllm_snapshot) and [this example with SGLang](/docs/examples/sglang_snapshot) for sample code, patterns, and other guidance.

#### GPU Memory Snapshots are generally incompatible with multi-GPU codeÂ
...
#### GPU Memory Snapshots are generally incompatible with non-CUDA GPU codeÂ

For instance, use of graphics capabilities prior to snapshotting will generally cause failures.

#### GPU Memory Snapshots do not speed up model loading from storageÂ

Memory Snapshots use the same high-performance distributed filesystem
that delivers Modal [Images](/docs/guide/images) and Modal [Volumes](/docs/guide/volumes) to our worldwide fleet of containers at minimum latency and maximum throughput.

That means that if the majority of your initialization latency is spent loading weights,
GPU Memory Snapshots will generally not improve your cold start times â
and may even worsen them, by adding overhead.
Instead, Memory Snapshots should primarily be used to âskip pastâ work
...
#### GPU Memory Snapshots can interact poorly with `torch.compile`Â
...
## Memory Snapshots FAQsÂ

 

### How do I know whether Memory Snapshots are being created or used?Â

You can see Memory Snapshots in action in your Functionâs âContainersâ tab. Containers that created a memory snapshot are marked with a  icon in the Startup column. Containers that restored from a snapshot are marked with a  icon. In the below screenshot, the container startup times when restoring from a memory snapshot are significantly faster.

![snapshot icons](https://modal-cdn.com/cdnbot/memory-snapshot-iconss6tm168n_cb303ec9.webp)

You can also search your Modal Appâs logs for the line `Snapshot created. Restoring Function from memory snapshot.`

### When are Memory Snapshots updated?Â
...
### I havenât changed my Function. Why do I still see Memory Snapshots being created sometimes?Â

Modal recaptures Memory Snapshots to keep up with the platformâs latest runtime and security changes.
...
### How do Memory Snapshots handle randomness?Â
...
## Advanced usage of Memory SnapshotsÂ

 

### Using GPUs without using GPU Memory SnapshotsÂ
...
#### GPUs are not available in CPU-only Memory SnapshotsÂ

If you are using the GPU Memory Snapshot feature (`enable_gpu_snapshot`), then
GPUs are available within `@modal.enter(snap=True)`.

If you are using memory snapshots *without* `enable_gpu_snapshot`, then itâs important
...
#### Watch out for accidental GPU initialization during CPU-only Memory SnapshotsÂ
...
If youâre using a framework that calls these methods during its import phase,
it may not be compatible with memory snapshots. The problem can manifest as
confusing âcuda not availableâ or âno CUDA-capable device is detectedâ errors.
...
[Memory Snapshots](#memory-snapshots)[CPU Memory Snapshots](#cpu-memory-snapshots)[Container lifecycle hooks and Memory Snapshots](#container-lifecycle-hooks-and-memory-snapshots)[GPU Memory Snapshots](#gpu-memory-snapshots)[Limitations of GPU Memory Snapshots](#limitations-of-gpu-memory-snapshots)[You may need to rewrite code for compatibility or to improve performance](#you-may-need-to-rewrite-code-for-compatibility-or-to-improve-performance)[GPU Memory Snapshots are generally incompatible with multi-GPU code](#gpu-memory-snapshots-are-generally-incompatible-with-multi-gpu-code)[GPU Memory Snapshots are generally incompatible with non-CUDA GPU code](#gpu-memory-snapshots-are-generally-incompatible-with-non-cuda-gpu-code)[GPU Memory Snapshots do not speed up model loading from storage](#gpu-memory-snapshots-do-not-speed-up-model-loading-from-storage)[GPU Memory Snapshots can interact poorly with torch.compile](#gpu-memory-snapshots-can-interact-poorly-with-torchcompile)[Memory Snapshots FAQs](#memory-snapshots-faqs)[How do I know whether Memory Snapshots are being created or used?](#how-do-i-know-whether-memory-snapshots-are-being-created-or-used)[When are Memory Snapshots updated?](#when-are-memory-snapshots-updated)[I havenât changed my Function. Why do I still see Memory Snapshots being created sometimes?](#i-havent-changed-my-function-why-do-i-still-see-memory-snapshots-being-created-sometimes)[How do Memory Snapshots handle randomness?](#how-do-memory-snapshots-handle-randomness)[Advanced usage of Memory Snapshots](#advanced-usage-of-memory-snapshots)[Using GPUs without using GPU Memory Snapshots](#using-gpus-without-using-gpu-memory-snapshots)[GPUs are not available in CPU-only Memory Snapshots](#gpus-are-not-available-in-cpu-only-memory-snapshots)[Watch out for accidental GPU initialization during CPU-only Memory Snapshots](#watch-out-for-accidental-gpu-initialization-during-cpu-only-memory-snapshots)