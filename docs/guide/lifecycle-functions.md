...
To accomplish this, you need to use Modalâs class syntax and the [`@app.cls`](/docs/sdk/py/latest/modal.App#cls) decorator. Specifically, youâll
need to:
...
3. Instead of the `@app.function` decorator on the original method, use `@modal.method` or the appropriate decorator for a [Web Function](#lifecycle-hooks-for-web-functions).
4. Add the correct method âhooksâ to your class based on your need:
...
## Lifecycle hooks for Web FunctionsÂ
...
[Container lifecycle hooks](#container-lifecycle-hooks)[@modal.enter](#modalenter)[@modal.exit](#modalexit)[Lifecycle hooks for Web Functions](#lifecycle-hooks-for-web-functions)