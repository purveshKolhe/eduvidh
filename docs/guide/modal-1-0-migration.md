...
## Deprecating `Image.copy_*` methodsÂ

*Introduced in: v0.72.11*

We recently introduced new `Image` methods â `Image.add_local_dir` and `Image.add_local_file` â to replace the existing `Image.copy_local_dir` and `Image.copy_local_file`.
...
## Deprecating `Mount` as part of the public APIÂ
...
## Deprecating the `@modal.build` decoratorÂ
...
weights. When using these frameworks, you just need to ensure that you mount a `modal.Volume` to the expected location of the frameworkâs cache:
...
## Requiring explicit inclusion of local Python dependenciesÂ

*Introduced in: 0.73.11* | *Enforced in: 1.0.0*

Prior to 1.0, Modal will inspect the modules that are imported when running
your App code and automatically include any âlocalâ modules in the remote
container environment. This behavior is referred to as âautomountingâ.
...
of your Modal App. The easiest way to do this is with [`Image.add_local_python_source`](/docs/sdk/py/latest/modal.Image#add_local_python_source):
...
package defining the App itself. Weâre introducing a new App or Function-level
...
## Renaming autoscaler parametersÂ

*Introduced in: v0.73.76*

Weâre renaming several parameters that configure autoscaling behavior:
...
Additionally, weâre promoting a fourth parameter, `buffer_containers`,
...
## Renaming `modal.web_endpoint` to `modal.fastapi_endpoint`Â

*Introduced in: v0.73.89*

Weâre renaming the `modal.web_endpoint` decorator to `modal.fastapi_endpoint` so that the implicit dependency on FastAPI is more clear. This can be a
...
## Replacing `allow_concurrent_inputs` with `@modal.concurrent`Â
...
Weâre moving the input concurrency feature out of âBetaâ status as part of this
change.
...
reducing latency during periods where the container pool is scaling up. See the [input concurrency guide](/docs/guide/concurrent-inputs) for more information.

## Deprecating the `.lookup` method on Modal objectsÂ
...
used, there is rarely any need to eagerly hydrate. Therefore, weâre deprecating `.lookup` so that thereâs only one obvious way to instantiate objects.

In most cases, the migration is a simple find-and-replace of `.lookup` â `.from_name`.
...
## Removing support for custom Cls constructorsÂ
...
Weâre making this change to address some persistent confusion about when
...
Additionally, weâre reducing the types that we support as class parameters to
...
## Simplifying Cls lookup patternsÂ
...
## Deprecating `modal.gpu` objectsÂ
...
## Requiring explicit invocation for module modeÂ
