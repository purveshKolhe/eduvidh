...
Source: https://modal.com/docs/guide/streaming-endpoints
...
Modal `fastapi_endpoint`s support streaming responses using FastAPIâs [`StreamingResponse`](https://fastapi.tiangolo.com/advanced/custom-response/#streamingresponse) class. This class accepts asynchronous generators, synchronous generators, or
...
This simple example combines Modalâs `@modal.fastapi_endpoint` decorator with a `StreamingResponse` object to produce a real-time SSE response.
...
## Streaming responses with `.remote`Â
...
## Streaming responses with `.map` and `.starmap`Â
...
ordered, but if this isnât necessary pass `order_outputs=False` as keyword
argument to the `.map` call.

### Asynchronous streamingÂ
...
* [An end-to-end example of streaming Youtube video transcriptions with OpenAIâs whisper model.](https://github.com/modal-labs/modal-examples/blob/main/06_gpu_and_ml/openai_whisper/streaming/main.py)
