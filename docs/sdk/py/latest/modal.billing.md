...
Source: https://modal.com/docs/sdk/py/latest/modal.billing
...
## modal.billing.WorkspaceBillingReportItemÂ
...
## modal.billing.workspace\_billing\_reportÂ
...
Modalâs billing model evolves.
...
be in UTC. Cost will be reported for full intervals, even if the provided `start` or `end` parameters are partial: `start` will be rounded to the beginning of its interval, while
...
and `tag_names` (i.e., keys) are specified in the request. Alternatively, pass `tag_names=["*"]` to include all tags in the report. Note that tags will be attributed to the entire interval even
...
Itâs also possible to generate reports using the [`modal billing report`](https://modal.com/docs/cli/latest/billing) CLI command. The CLI
has a few convenience features for generating reports across relative time ranges.

[modal.billing](#modalbilling)[WorkspaceBillingReportItem](#modalbillingworkspacebillingreportitem)[workspace\_billing\_report](#modalbillingworkspace_billing_report)